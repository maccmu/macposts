...
MNM_Realtime_Dta::initialize ()
{
  ...
  // num_path); MNM_IO::hook_up_od_node(m_file_folder, m_dta_conf_reader,
  // m_od_factory, m_before_shot -> node_factory);
  ...
  MNM_IO::build_vms_facotory (m_file_folder, m_graph,
                              m_realtime_dta_config->get_int ("num_vms"),
                              m_vms_factory);
...
int
MNM_Realtime_Dta::init_running ()
{
  if (m_before_shot != NULL)
    delete m_before_shot;
  if (m_after_shot != NULL)
    delete m_after_shot;
  m_before_shot = MNM::make_empty_screenshot (m_file_folder, m_dta_config,
                                              m_od_factory, m_graph);
  m_after_shot = MNM::make_empty_screenshot (m_file_folder, m_dta_config,
                                             m_od_factory, m_graph);
  ...
  Path_Table *path_table, std::unordered_map<TInt, TFlt> *link_spd_map)
{
  ...
  MNM_IO::hook_up_od_node (m_file_folder, m_dta_config, m_od_factory,
                           _node_factory);
  printf ("Finish prepare routing\n");
  // m_statistics -> init_record();
  for (auto _node_it = _node_factory->m_node_map.begin ();
       _node_it != _node_factory->m_node_map.end (); _node_it++)
    {
  ...
  while (_cur_inter < _total_inter)
    {
      ...
      if (_cur_inter == 0)
        {
          for (auto _origin_it = m_od_factory->m_origin_map.begin ();
               _origin_it != m_od_factory->m_origin_map.end (); _origin_it++)
            {
              ...
              _origin->release_one_interval (_real_inter, _veh_factory,
                                             assign_inter, TFlt (0));
            }
        }
      ...
      for (auto _link_it = _link_factory->m_link_map.begin ();
  ...
  for (auto _it : *path_table)
    {
      for (auto _it_it : *(_it.second))
        {
          _demand = MNM::get_demand_bynode (_it.first, _it_it.first,
                                            assign_inter, _node_factory);
          // printf("Demand is %.4f\n", _demand);
          for (MNM_Path *_path : _it_it.second->m_path_vec)
            {
              _tmp_grad = 0;
...
int
MNM_Realtime_Dta::get_optimization_gradient (MNM_Dta_Screenshot *screenshot,
...
int
MNM_Realtime_Dta::one_iteration (TInt assign_inter)
{
  ...
  MNM::generate_vms_instructions (_vms_info, m_vms_factory,
                                  m_after_shot->m_link_factory);
  return 0;
}

int
MNM_Realtime_Dta::update_compliance_ratio (TInt assign_inter)
{
  TFlt Learning_Rate = 0.3;
  // m_p real p in last interval, buffer[0] estimated best in last interval
  // ratio =  (estimated_flow  - real_flow) / estimated_flow   on non_detour
  // links
  TInt _link_ID;
...
int
MNM_Realtime_Dta::update_vms (TInt next_assign_inter)
{
  for (auto _map_it : m_vms_factory->m_link_vms_map)
    {
      MNM_Link_Vms *_vms = _map_it.second;
      _vms->generate_detour_link (m_path_table, next_assign_inter,
                                  m_before_shot->m_node_factory);
    }
  return 0;
}
...
int
MNM_Realtime_Dta::estimate_previous (TInt assign_inter)
{
  ...
  for (int i = 0; i < m_estimation_iters; ++i)
    {
      ...
      get_estimation_gradient (_tmp_shot, m_estimation_length, assign_inter,
                               m_path_table, &_link_spd_map);
      // for(auto _it : *m_path_table){
      //   for (auto _it_it : *(_it.second)){
      //     for (MNM_Path* _path : _it_it.second -> m_path_vec){
      ...
      MNM::update_path_p (m_path_table, 1,
                          m_estimation_step_size / TFlt (i + 1));
...
int
MNM_Realtime_Dta::optimize_next (TInt next_assign_inter)
{
  MNM_Dta_Screenshot *_tmp_shot;
  for (int i = 0; i < m_optimization_iters; ++i)
    {
      ...
      get_optimization_gradient (_tmp_shot, m_estimation_length,
                                 next_assign_inter, m_path_table);
      printf ("optimize_next::update_path_p\n");
      MNM::update_path_p (m_path_table, 3,
                          m_optimization_step_size / TFlt (i + 1));
...
int
MNM_Realtime_Dta::predict_next (TInt next_assign_inter)
{
  TInt _cur_inter = 0;
  MNM_Origin *_origin;
  MNM_Dnode *_node;
      ...
      if ((_cur_inter + 1) % _assign_freq == 0)
        {
          for (auto _origin_it = m_od_factory->m_origin_map.begin ();
               _origin_it != m_od_factory->m_origin_map.end (); _origin_it++)
            {
              ...
              _origin->release_one_interval (_cur_inter, _veh_factory,
...
MNM_Statistics *
MNM_Realtime_Dta::get_statistics (MNM_Node_Factory *node_factory,
                                  MNM_Link_Factory *link_factory)
{
  ...
  if (_record_config->get_string ("rec_mode") == "LRn")
    ...
    {
      throw std::runtime_error ("unsupported rec mode");
    }
}
/**************************************************************************
                          Screen shot
...
int
MNM_Dta_Screenshot::build_static_network ()
{
...
int
MNM_Dta_Screenshot::hook_up_node_and_link ()
{
  TInt _node_ID;
  MNM_Dnode *_node;
  MNM_Dlink *_link;
  ...
  while (_cur_inter < _total_inter)
    {
      ...
      if (_cur_inter == 0)
        {
          for (auto _origin_it = od_factory->m_origin_map.begin ();
               _origin_it != od_factory->m_origin_map.end (); _origin_it++)
            {
              _origin = _origin_it->second;
              _origin->release_one_interval (_real_inter, _veh_factory,
                                             assign_inter, TFlt (0));
      ...
      // printf("Receiving!\n");
      // step 5: Destination receive vehicle
      for (auto _dest_it = od_factory->m_destination_map.begin ();
           _dest_it != od_factory->m_destination_map.end (); _dest_it++)
        {
          _dest = _dest_it->second;
          _dest->receive (_real_inter);
        }
       ...
       _link_it != link_factory->m_link_map.end (); _link_it++)
    {
      ...
      if (MNM_Dlink_Ctm *_ctm = dynamic_cast<MNM_Dlink_Ctm *> (_dlink))
      ...
      else if (MNM_Dlink_Pq *_pq = dynamic_cast<MNM_Dlink_Pq *> (_dlink))
        {
          ...
          for (auto _veh_it = _pq->m_veh_queue.begin ();
...
int
update_path_p (Path_Table *path_table, TInt col, TFlt step_size)
{
