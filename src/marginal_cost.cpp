...
int
MNM_Link_Tt::update_tt (TInt timestamp)
{
  MNM_Dlink *_link;
          ...
          if (m_tracker.find (_veh) == m_tracker.end ())
          ...
          else
            {
              ...
              if (_veh->m_finish_time > 0)
                {
                  update_indiviual_tt (_record->m_link,
                                       _duration * m_unit_time);
                  delete _record;
...
int
MNM_Link_Tt::change_link (MNM_Veh *veh, MNM_Dlink *link, TInt current_time)
{
...
int
MNM_Link_Tt::update_indiviual_tt (MNM_Dlink *link, TFlt tt)
{
