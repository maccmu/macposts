...
MNM_Veh_Electrified::MNM_Veh_Electrified (TInt ID, TInt start_time,
                                          TFlt starting_range,
                                          bool using_roadside_charging,
                                          TFlt full_range)
  ...
  if (m_using_roadside_charging)
    {
      ...
      if (m_current_range - m_miles_traveled_after_charging
          <= 0.1 * m_full_range)
        {
          return true;
        }
...
MNM_Veh_Electrified *
MNM_Veh_Factory_EV::make_veh_electrified (TInt timestamp, Vehicle_type veh_type,
                                          bool using_roadside_charging)
{
  TFlt starting_range = using_roadside_charging
                          ? m_starting_range_roadside_charging
                          : m_starting_range_non_roadside_charging;
  TFlt full_range = m_full_range;
  MNM_Veh_Electrified *_veh
    = new MNM_Veh_Electrified (m_num_veh + 1, timestamp, starting_range,
                               using_roadside_charging, full_range);
...
MNM_Veh_Factory_EV::make_veh_electrified_delivery (TInt timestamp,
                                                   ...
                                                   bool using_roadside_charging)
{
  TFlt starting_range = using_roadside_charging
                          ...
                          : m_starting_range_non_roadside_charging;
  TFlt full_range = m_full_range;
  MNM_Veh_Electrified_Delivery *_veh
    = new MNM_Veh_Electrified_Delivery (m_num_veh + 1, timestamp,
                                        starting_range, using_roadside_charging,
                                        full_range);
  _veh->m_type = veh_type;
  m_veh_map.insert (std::pair<TInt, MNM_Veh *> (m_num_veh + 1, _veh));
  m_num_veh += 1;
...
int
MNM_Veh_Factory_EV::set_ev_range (TFlt EV_starting_range_roadside_charging,
                                  TFlt EV_starting_range_non_roadside_charging,
                                  TFlt EV_full_range)
{
  m_starting_range_roadside_charging = EV_starting_range_roadside_charging;
  m_starting_range_non_roadside_charging
    = EV_starting_range_non_roadside_charging;
...
MNM_Origin_EV::MNM_Origin_EV (TInt ID, TInt max_interval, TFlt flow_scalar,
                              TInt frequency, TInt ev_label,
                              TFlt roadside_charging_ratio)
    : MNM_Origin_Delivery::MNM_Origin_Delivery (ID, max_interval, flow_scalar,
                                                frequency)
{
  IAssert (ev_label >= 0);
  m_ev_label = ev_label;
      ...
      if (auto _veh_elec = dynamic_cast<MNM_Veh_Electrified_Delivery *> (_it))
        {
          ...
          // std::cout << "EV with multi OD pairs needs charging after first
          ...
          if (_veh_elec->need_charging ())
          ...
          else
            {
              ...
              // once the veh needs charging, the m_type is changed to adaptive
              // for the remaing OD sequence travel this step can mitigate this
...
int
MNM_Origin_EV::release_one_interval (TInt current_interval,
      ...
      for (int i = 0; i < _veh_to_release; ++i)
        {
          ...
          if (m_ev_label >= 0 && _label == m_ev_label)
            {
              ...
              if (_r <= m_roadside_charging_ratio)
                {
                  // EV using roadside charging station and it will be routed to
                  // charging station during its trip
                  _veh
                    = _veh_factory_ev->make_veh_electrified (current_interval,
                                                             MNM_TYPE_ADAPTIVE,
                                                             true);
                }
              else
                {
                  ...
                  _veh = _veh_factory_ev
                           ->make_veh_electrified (current_interval,
                                                   MNM_Ults::rand_flt ()
                                                       <= adaptive_ratio
                                                     ? MNM_TYPE_ADAPTIVE
                                                     : MNM_TYPE_STATIC,
            ...
            {
              // non-EV
              _veh = _veh_factory_ev->make_veh (current_interval,
                                                MNM_Ults::rand_flt ()
                                                    <= adaptive_ratio
                                                  ? MNM_TYPE_ADAPTIVE
                                                  : MNM_TYPE_STATIC);
          ...
          if (dynamic_cast<MNM_Veh_Electrified *> (_veh) != nullptr)
            {
              if (dynamic_cast<MNM_Veh_Electrified *> (_veh)
                    ->m_using_roadside_charging)
                {
                  // std::cout << "EV with single OD pair needs charging: " <<
                  // dynamic_cast<MNM_Veh_Electrified*>(_veh) -> need_charging()
                  // << std::endl;
                  Assert (dynamic_cast<MNM_Veh_Electrified *> (_veh)
                            ->need_charging ());
                }
            }
      ...
      for (int i = 0; i < _veh_to_release; ++i)
        {
          ...
          if (m_ev_label >= 0 && _label == m_ev_label)
            {
              _veh = _veh_factory_ev->make_veh_electrified_delivery (
                current_interval,
                MNM_Ults::rand_flt () <= adaptive_ratio ? MNM_TYPE_ADAPTIVE
                                                        : MNM_TYPE_STATIC,
            ...
            {
              // non-EV
              _veh = _veh_factory_ev->make_veh_delivery (current_interval,
                                                         MNM_Ults::rand_flt ()
                                                             <= adaptive_ratio
                                                           ? MNM_TYPE_ADAPTIVE
                                                           : MNM_TYPE_STATIC);
          ...
          dynamic_cast<MNM_Veh_Delivery *> (_veh)->set_multi_od_seq (
            _demand_it.first); // will be used in need_charging()
          // first destination
          ...
          if (dynamic_cast<MNM_Veh_Electrified_Delivery *> (_veh) != nullptr)
            {
              if (dynamic_cast<MNM_Veh_Electrified_Delivery *> (_veh)
                    ->m_using_roadside_charging)
                {
                  // std::cout << "EV with multi-OD pairs needs charging in its
                  // first OD trip: " <<
                  // dynamic_cast<MNM_Veh_Electrified*>(_veh) -> need_charging()
                  // << std::endl;
                  if (dynamic_cast<MNM_Veh_Electrified_Delivery *> (_veh)
...
MNM_Origin *
MNM_OD_Factory_EV::make_origin_ev (TInt ID, TInt max_interval, TFlt flow_scalar,
                                   TInt frequency, TInt ev_label,
                                   TFlt roadside_charging_ratio)
{
  MNM_Origin *_origin
    = new MNM_Origin_EV (ID, max_interval, flow_scalar, frequency, ev_label,
                         roadside_charging_ratio);
...
MNM_Charging_Station::save_cc (const std::string &file_name)
{
  ...
  if (!_file.is_open ())
    {
      std::cout << "MNM_Charging_Station::save_cc, Error happens when open "
                << file_name << std::endl;
...
MNM_Charging_Station::save_waiting_time_record (const std::string &file_name)
{
  ...
  if (!_file.is_open ())
    {
      std::cout << "MNM_Charging_Station::save_waiting_time_record, Error "
                   "happens when open "
                << file_name << std::endl;
      throw std::runtime_error ("Error");
    }
  ...
  for (auto i : _k)
    {
      ...
      _s += std::to_string (_it.first) + ","
            + std::to_string (_it.second * m_unit_time) + "\n";
    }
  _s.pop_back ();
  _file << _s;
  if (_file.is_open ())
...
MNM_Charging_Station::add_in_link (MNM_Dlink *in_link)
{
  if (auto _in_link_pq = dynamic_cast<MNM_Dlink_Pq *> (in_link))
    ...
    {
      throw std::runtime_error ("MNM_Charging_Station::add_in_link, incoming "
                                "link for charging station must be PQ link");
    }
  return 0;
}
...
MNM_Charging_Station::add_out_link (MNM_Dlink *out_link)
{
  if (auto _out_link_pq = dynamic_cast<MNM_Dlink_Pq *> (out_link))
    ...
    {
      throw std::runtime_error ("MNM_Charging_Station::add_out_link, outgoing "
                                "link for charging station must be PQ link");
    }
  return 0;
}
  ...
  for (auto _queue : m_queue_pool)
    {
      for (auto _veh : _queue)
        {
          // if (dynamic_cast<MNM_Veh_Electrified*>(_veh) == nullptr) {
          // if (dynamic_cast<MNM_Veh_Electrified*>(_veh) -> m_charging_station
          // == nullptr) {
          if (std::find (m_out_link_array.begin (), m_out_link_array.end (),
                         _veh->get_next_link ())
              == m_out_link_array.end ())
            {
              // if (_veh -> get_current_link() == nullptr || _veh ->
              // m_finish_time > 0) { if (_veh -> get_current_link() == nullptr
              // || _veh -> get_next_link() == nullptr || _veh -> m_finish_time
              // > 0) {
...
int
MNM_Charging_Station::evolve (TInt timestamp)
{
      ...
      while (_veh_it != _in_link->m_finished_array.end ())
        {
          if (std::find (m_out_link_array.begin (), m_out_link_array.end (),
              ...
              == m_out_link_array.end ())
            {
              printf ("MNM_Charging_Station::evolve, vehicle in the wrong "
                      "node, no exit!\n");
          ...
          if (_veh == nullptr || _veh->m_type != MNM_TYPE_ADAPTIVE
              || _veh->m_charging_station != this)
            {
              throw std::runtime_error ("wrong routing for this vehicle");
              // TODO: actually non-EV or ev using other charging station should
              // not be here move_out_veh(timestamp, *_veh_it); _veh_it =
              // _in_link->m_finished_array.erase(_veh_it); continue;
            }
          // if (_veh -> need_charging() && _veh -> m_charging_station != this)
          // {
          //     throw std::runtime_error("MNM_Charging_Station::evolve,
          //     electrified vehicle in the wrong charging station\n");
          // }

          // entering the shortest queue
          auto _result
            = std::min_element (m_queue_pool.begin (), m_queue_pool.end (),
                                [] (const std::deque<MNM_Veh *> &q1,
                                    const std::deque<MNM_Veh *> &q2) {
                                  return q1.size () < q2.size ();
                                });
          m_queue_pool.at (std::distance (m_queue_pool.begin (), _result))
            .push_back (_veh);
  ...
  for (size_t i = 0; i < m_queue_pool.size (); ++i)
    {
      auto _queue = m_queue_pool.at (i); // queue is actually a copy, not the
                                         // original element of m_queue_pool
      if (_queue.empty ())
        continue;
      auto _veh = dynamic_cast<MNM_Veh_Electrified *> (_queue.front ());
      if (_veh->m_charging_timer < 0)
        _veh->m_charging_timer = 0;
      if (_veh->m_charging_timer >= m_charging_time)
        {
          ...
          m_queue_pool.at (i)
            .pop_front (); // DO NOT use _queue.pop_front(), which will not
                           // modify m_queue_pool
          move_out_veh (timestamp, _veh);
          // update charging station cc
          m_N_out->add_increment (
  ...
  m_waiting_time_record.insert (
    std::pair<int, int> (timestamp + 1, _est_waiting_time));
  return 0;
}
...
{
  std::deque<MNM_Veh *> _shortest_queue
    = *std::min_element (m_queue_pool.begin (), m_queue_pool.end (),
                         [] (const std::deque<MNM_Veh *> &q1,
                             const std::deque<MNM_Veh *> &q2) {
                           return q1.size () < q2.size ();
                         });
  // int _est_waiting_time = _shortest_queue.size() > 0 ? _shortest_queue.size()
  // * m_charging_time : m_avg_waiting_time; if (_est_waiting_time <
  // m_avg_waiting_time) _est_waiting_time = m_avg_waiting_time;
...
{
  auto _in_link = veh->get_current_link (); // veh will be popped out from
                                            // in_link outside this function
  ...
  if (_out_link->m_N_in_tree != nullptr)
    {
      // printf("record out link cc tree: link ID %d, time %d, path id %d,
      // assign interval %d\n", _out_link -> m_link_ID(), timestamp()+1, _veh ->
      // m_path -> m_path_ID(), _veh -> m_assign_interval());
      _out_link->m_N_in_tree->add_flow (TFlt (timestamp + 1),
                                        TFlt (1) / m_flow_scalar, veh->m_path,
                                        veh->m_assign_interval);
    }
  if (_in_link->m_N_out_tree != nullptr)
    {
      // printf("record in link cc tree: link ID %d, time %d, path id %d, assign
      // interval %d\n", _in_link -> m_link_ID(), timestamp()+1, _veh -> m_path
      // -> m_path_ID(), _veh -> m_assign_interval());
      _in_link->m_N_out_tree->add_flow (TFlt (timestamp + 1),
                                        TFlt (1) / m_flow_scalar, veh->m_path,
                                        veh->m_assign_interval);
    }
  return 0;
  ...
  if (m_node_map.empty ())
    {
      throw std::runtime_error (
        "MNM_Node_Factory_With_EV::make_charging_station, must add node first");
    }
  if (m_node_map.find (ID) != m_node_map.end ())
    {
      throw std::runtime_error (
        "MNM_Node_Factory_With_EV::make_charging_station, charging station ID "
        "must be different from any existing node ID");
    }
  MNM_Dnode *_node
    = new MNM_Charging_Station (ID, flow_scalar, unit_time, num_slots,
                                charging_time, avg_waiting_time, price);
...
{
  // hopefully this will not route non-EV to charging station since extra delay
  // is added at charging station
  get_POIs ();
  m_od_candidate_poi_table = od_candidate_poi_table;
  if (m_od_candidate_poi_table->empty ())
    {
      construct_full_od_candidate_poi_table (); // use all possible OD pairs
                                                // from OD_factory
    }
  ...
  for (auto _it : *m_od_candidate_poi_table)
    {
      if (m_best_poi_table->find (_it.first) == m_best_poi_table->end ())
        {
          m_best_poi_table->insert (
            std::make_pair (_it.first, new std::unordered_map<MNM_Destination *,
                                                              MNM_Dnode *> ()));
        }
      for (auto _it_it : *(_it.second))
        {
          if (m_best_poi_table->find (_it.first)->second->find (_it_it.first)
              == m_best_poi_table->find (_it.first)->second->end ())
            {
              m_best_poi_table->find (_it.first)->second->insert (
                std::make_pair (_it_it.first, nullptr));
            }
        }
    }
}
...
MNM_Routing_Adaptive_With_POIs::get_POIs ()
{
  ...
  for (auto _it : m_node_factory->m_node_map)
    {
      if (auto _charging_station
          = dynamic_cast<MNM_Charging_Station *> (_it.second))
        {
  ...
  for (auto _it1 : m_od_factory->m_origin_map)
    {
      ...
      for (auto _it2 : m_od_factory->m_destination_map)
        {
          _dest = _it2.second;
          if (m_od_candidate_poi_table->find (_origin)
              == m_od_candidate_poi_table->end ())
            {
              m_od_candidate_poi_table->insert (
                std::make_pair (_origin, new std::unordered_map<
                                           MNM_Destination *,
                                           std::vector<MNM_Dnode *> *> ()));
            }
          if (m_od_candidate_poi_table->find (_origin)->second->find (_dest)
              == m_od_candidate_poi_table->find (_origin)->second->end ())
            {
              m_od_candidate_poi_table->find (_origin)->second->insert (
                std::make_pair (_dest, new std::vector<MNM_Dnode *> ()));
            }
          for (auto _charging_station : m_mid_POIs)
            {
              m_od_candidate_poi_table->find (_origin)
                ->second->find (_dest)
                ->second->push_back (_charging_station);
            }
        }
    }
  return 0;
      ...
      for (auto _it_it : *(_it.second))
        {
          ...
          //     MNM_Shortest_Path::all_to_one_FIFO(_node -> m_node_ID, m_graph,
          //     _cost_map, _shortest_path_tree); if
          //     (_shortest_path_tree.find(_origin -> m_origin_node ->
          //     m_node_ID)-> second == -1){
          //         printf("Disconnectivity in Origin %d (Node: %d) -
          //         Destination %d (Node: %d) pair, cannot travel from Origin
          //         %d (Node: %d) to POI Node %d",
          //             _origin -> m_Origin_ID(), _origin -> m_origin_node ->
          //             m_node_ID(), _dest -> m_Dest_ID(), _dest -> m_dest_node
          //             -> m_node_ID(), _origin -> m_Origin_ID(), _origin ->
          //             m_origin_node -> m_node_ID(), _node -> m_node_ID());
          //         return false;
          //     }

          //     // _node to _dest
          //     if (!_shortest_path_tree.empty()) _shortest_path_tree.clear();
          //     MNM_Shortest_Path::all_to_one_FIFO(_dest -> m_dest_node ->
          //     m_node_ID, m_graph, _cost_map, _shortest_path_tree); if
          //     (_shortest_path_tree.find(_node -> m_node_ID)-> second == -1){
          //         printf("Disconnectivity in Origin %d (Node: %d) -
          //         Destination %d (Node: %d) pair, cannot travel from POI Node
          //         %d to Destination %d (Node: %d)",
          //             _origin -> m_Origin_ID(), _origin -> m_origin_node ->
          //             m_node_ID(), _dest -> m_Dest_ID(), _dest -> m_dest_node
          //             -> m_node_ID(), _node -> m_node_ID(), _dest ->
          //             m_Dest_ID(), _dest -> m_dest_node -> m_node_ID());
          //         return false;
          //     }
...
MNM_Routing_Adaptive_With_POIs::init_routing (Path_Table *path_table)
{
  if (m_statistics->m_self_config->get_int ("rec_tt") == 0)
    {
      throw std::runtime_error (
        "MNM_Routing_Adaptive_With_POIs::init_routing, rec_tt should be set to "
        "1 in config.conf to use adaptive routing");
    }
  ...
  for (auto _it : m_mid_POIs)
    {
      ...
      m_table_POIs->insert (
        std::pair<MNM_Dnode *,
                  std::unordered_map<TInt, TInt> *> (_it, _shortest_path_tree));
    }
  return 0;
}
  ...
  for (auto _it : m_statistics->m_record_interval_tt)
    { // seconds
      ...
      if (dynamic_cast<MNM_Dlink_Pq *> (_link) != nullptr)
        {
          if (auto _charging_station
              = dynamic_cast<MNM_Charging_Station *> (_link->m_to_node))
            {
              // this link cost is only for routing, it is amplified by a factor
              // to discourage non-EV from using this link
              m_link_cost[_it.first]
                += (m_vot
                      * (_charging_station
                           ->get_current_estimated_waiting_time ()
                         + _charging_station->m_charging_time)
                      * _charging_station->m_unit_time
...
int
MNM_Routing_Adaptive_With_POIs::set_shortest_path_tree (
  std::unordered_map<TInt, TInt> **shortest_path_tree, MNM_Dnode *poi_node,
  MNM_Destination *veh_dest)
{
  if (poi_node != nullptr && veh_dest == nullptr)
    {
      if (m_table_POIs->find (poi_node) == m_table_POIs->end ())
        {
          throw std::runtime_error (
            "MNM_Routing_Adaptive_With_POIs::set_shortest_path_tree, cannot "
            "find middle node in m_table_POIs");
        }
    ...
    {
      throw std::runtime_error (
        "MNM_Routing_Adaptive_With_POIs::set_shortest_path_tree, either "
        "poi_node or veh_dest can be nullptr, cannot be both");
    }
  return 0;
}
          ...
          if (_veh->m_type == MNM_TYPE_ADAPTIVE)
            {
              ...
              if (auto _veh_electrified
                  = dynamic_cast<MNM_Veh_Electrified *> (_veh))
                {
                  if (_veh_electrified->need_charging ())
                    {
                      if (m_best_poi_table->find (_origin)
                            == m_best_poi_table->end ()
                          || m_best_poi_table->find (_origin)->second->find (
                               _veh_dest)
                               == m_best_poi_table->find (_origin)
                                    ->second->end ())
                        {
                          set_shortest_path_tree (&_shortest_path_tree, nullptr,
                                                  _veh_dest);
                        }
                      else
                        {
                          _poi_node = m_best_poi_table->find (_origin)
                                        ->second->find (_veh_dest)
                                        ->second;
                          if (_poi_node != nullptr)
                            {
                              _veh_electrified->m_charging_station
                                = dynamic_cast<MNM_Charging_Station *> (
                                  _poi_node);
                              set_shortest_path_tree (&_shortest_path_tree,
                                                      _poi_node, nullptr);
                            }
                          else
                            {
              ...
              if (_next_link_ID < 0)
                {
                  ...
                  // it.second);
                  throw std::runtime_error (
                    "MNM_Routing_Adaptive_With_POIs::update_routing, Something "
                    "wrong in adaptive routing, wrong next link 1");
                }
          ...
          if (_veh->m_type == MNM_TYPE_ADAPTIVE)
            {
              if (_link != _veh->get_current_link ())
                {
                  throw std::runtime_error (
                    "MNM_Routing_Adaptive_With_POIs::update_routing, wrong "
                    "current link");
                }
              ...
              if (_veh_dest->m_dest_node->m_node_ID == _node_ID)
              ...
              else
                {
                      ...
                      if (_veh_electrified->need_charging ())
                        {
                          if (_veh_electrified->m_charging_station != nullptr)
                            {
                              if (_veh_electrified->m_charging_station
                                    ->m_node_ID
                                  != _node_ID)
                                {
                                  set_shortest_path_tree (
                                    &_shortest_path_tree,
                                    _veh_electrified->m_charging_station,
                                    nullptr);
                                }
                        ...
                        {
                          Assert (_veh_electrified->m_charging_station
                                  == nullptr);
                          set_shortest_path_tree (&_shortest_path_tree, nullptr,
                                                  _veh_dest);
                        }
                  ...
                  if (_next_link_ID == -1)
                    {
                      printf (
                        "MNM_Routing_Adaptive_With_POIs::update_routing, "
                        "Something wrong in routing, wrong next link 2\n");
                      printf ("MNM_Routing_Adaptive_With_POIs::update_routing, "
                              "The node is %d, the vehicle should head to %d\n",
                              (int) _node_ID,
                      ...
                      if (deg > 0)
                      ...
                      else
                        {
                          throw std::runtime_error (
                            "MNM_Routing_Adaptive_With_POIs::update_routing, "
                            "cannot do anything to find a next link");
                        }
                    }
                  _next_link = m_link_factory->get_link (_next_link_ID);
                  if (_next_link != nullptr)
                    {
                          ...
                          if (_veh_electrified->need_charging ())
                            {
                              if (_veh_electrified->m_charging_station
                                ...
                                {
                                  if (_next_node_ID
                                      != _veh_dest->m_dest_node->m_node_ID)
                                    {
                                      _shortest_path_tree
                                        = m_table->find (_veh_dest)->second;
                                    }
                                }
                            }
                          else
                      ...
                      if (_shortest_path_tree != nullptr)
                        {
                          if (_shortest_path_tree->find (_next_node_ID)
                              == _shortest_path_tree->end ())
                            {
                              throw std::runtime_error (
                                "MNM_Routing_Adaptive_With_POIs::update_"
                                "routing, Cannot find _next_node_ID");
                            }
                          if (_shortest_path_tree->find (_next_node_ID)->second
                              == -1)
                            {
                              throw std::runtime_error (
                                "MNM_Routing_Adaptive_With_POIs::update_"
                                "routing, Something wrong for the future node");
                            }
      ...
      for (auto _it_it : *(_it.second))
        {
          ...
          if (_it_it.second->empty ())
            {
              // printf("MNM_Routing_Adaptive_With_POIs::update_best_POI, empty
              // candidate POI list\n"); exit(-1);
              m_best_poi_table->find (_origin)->second->find (_dest)->second
                = _current_best_poi_node;
              continue;
            }
          ...
          for (auto _node : *(_it_it.second))
            {
              ...
              _shortest_path_tree = m_table->find (_dest)->second;
              _path = nullptr;
              _path = MNM::extract_path (_node->m_node_ID,
                                         _dest->m_dest_node->m_node_ID,
                                         *_shortest_path_tree, m_graph);
              ...
              _path = nullptr;
              _path = MNM::extract_path (_origin->m_origin_node->m_node_ID,
                                         _node->m_node_ID, *_shortest_path_tree,
                                         m_graph);
              Assert (_path != nullptr);
              _path_cost += MNM::get_path_tt_snapshot (_path, m_link_cost);
              delete _path;
      ...
      for (auto _it_it : *(_it.second))
        {
          ...
          for (auto _node : *(_it_it.second))
            {
              _path_cost = dynamic_cast<MNM_Charging_Station *> (_node)
                             ->get_current_estimated_waiting_time ();
              if (_current_best_path_cost > _path_cost)
                {
                  _current_best_poi_node = _node;
    ...
    : MNM_Routing::MNM_Routing (graph, od_factory, node_factory, link_factory)
{
  m_routing_fixed
    = new MNM_Routing_Delivery_Fixed (graph, od_factory, node_factory,
                                      ...
                                      buffer_len);
  m_routing_adaptive
    = new MNM_Routing_Adaptive_With_POIs (file_folder, graph, statistics,
                                          od_factory, node_factory,
                                          link_factory, od_candidate_poi_table);
...
int
MNM_IO_EV::build_od_factory_ev (const std::string &file_folder,
      ...
      for (int i = 0; i < _num_of_O; ++i)
        {
          ...
          _words = split (_line, ' ');
          // O_ID, node_ID, (pickup_waiting_time (seconds),
          // roadside_charging_ratio)
          if (_words.size () == 2 || _words.size () == 3 || _words.size () == 4)
            {
              ...
              if (_words.size () == 3)
                ...
                {
                  throw std::runtime_error (
                    "Something is wrong in origin part in od input file");
                }
              _origin
                = _od_factory_ev->make_origin_ev (_origin_ID, _max_interval,
                                                  _flow_scalar, _frequency,
                                                  _ev_label,
                                                  _roadside_charging_ratio);
              dynamic_cast<MNM_Origin_EV *> (_origin)->m_pickup_waiting_time
      ...
      for (int i = 0; i < _num_of_D; ++i)
        {
          ...
          if (_words.size () == 2)
            ...
            {
              throw std::runtime_error (
                "Something is wrong in destination part in od input file");
            }
        }
    }
  _od_file.close ();
...
int
MNM_IO_EV::add_charging_station_node (const std::string &file_folder,
                                      MNM_ConfReader *conf_reader,
                                      MNM_Node_Factory *node_factory,
                                      const std::string &file_name)
{
  ...
  if (_file.is_open ())
    {
      TInt _num_of_charging_station
        = conf_reader->get_int ("num_of_charging_station");
      ...
      std::getline (_file, _line); // #charging_station_ID num_slots
                                   // avg_charging_time avg_waiting_time price
          ...
          if (TInt (_words.size ()) == 5)
            {
              ...
              _charging_time = int (std::stof (trim (_words[2]))
                                    / _unit_time); // seconds -> intervals
              _avg_waiting_time = int (std::stof (trim (_words[3]))
                                       / _unit_time); // seconds -> intervals
              _price = std::stof (trim (_words[4]));  // US dollar
              // add charging station to node factory
              _node_factory_ev->make_charging_station (_ID, _flow_scalar,
                                                       _unit_time, _num_slots,
                                                       _charging_time,
                                                       _avg_waiting_time,
                                                       _price);
            }
            ...
            {
              throw std::runtime_error (
                "Something wrong in MNM_IO_With_EV::add_charging_station_node");
            }
        }
      _file.close ();
...
// refer to
// /home/qlzou/Documents/CodeBase/maccmu_github/macposts-dev-examples/cpp/test_candidate_charging_station.cpp
// if not, it tries to use all POIs, which can be slow
OD_Candidate_POI_Table *
MNM_IO_EV::load_candidate_poi_table (const std::string &file_folder,
                                     MNM_ConfReader *conf_reader,
                                     MNM_OD_Factory *od_factory,
                                     MNM_Node_Factory *node_factory,
                                     const std::string &file_name)
{
  auto _node_factory_with_ev
    = dynamic_cast<MNM_Node_Factory_EV *> (node_factory);
          ...
          if (_words.size () >= 2)
            {
              ...
              if (table->find (_origin)->second->find (_dest)
                  == table->find (_origin)->second->end ())
                {
                  table->find (_origin)->second->insert (
                    std::make_pair (_dest, new std::vector<MNM_Dnode *> ()));
                }
              if (_words.size () == 2)
                continue;
              auto _candidate_poi_vec
                = table->find (_origin)->second->find (_dest)->second;
              for (size_t i = 2; i < _words.size (); ++i)
                {
                  _charging_station_ID = TInt (std::stoi (trim (_words[i])));
                  _charging_station = dynamic_cast<MNM_Charging_Station *> (
                    _node_factory_with_ev->get_node (_charging_station_ID));
                  Assert (_charging_station != nullptr);
                  auto _it
                    = std::find (_candidate_poi_vec->begin (),
                                 _candidate_poi_vec->end (), _charging_station);
          ...
          else
            {
              throw std::runtime_error (
                "Something wrong in "
                "MNM_IO_With_EV::build_candidate_charging_station");
            }
        }
...
int
MNM_IO_EV::save_candidate_poi_table (
  const OD_Candidate_POI_Table &candidate_poi_table,
  const std::string &file_name)
{
  std::ofstream _file;
  _file.open (file_name, std::ofstream::out);
  if (!_file.is_open ())
    {
      std::cout << "MNM_Charging_Station::save_candidate_poi_table, Error "
                   "happens when open "
                << file_name << std::endl;
      throw std::runtime_error ("Error");
    }
  ...
  for (auto _it : candidate_poi_table)
    {
      for (auto _it_it : *_it.second)
        {
          _str = std::to_string (_it.first->m_Origin_ID) + " "
                 + std::to_string (_it_it.first->m_Dest_ID) + " ";
...
int
MNM_IO_EV::save_charging_station_record (const std::string &file_folder,
                                         MNM_Node_Factory *node_factory)
{
  for (auto _node_it : node_factory->m_node_map)
    {
      if (auto _charging_station
          = dynamic_cast<MNM_Charging_Station *> (_node_it.second))
        {
...
MNM_Dta_EV::build_from_files ()
{
  ...
  MNM_IO_EV::add_charging_station_node (m_file_folder, m_config,
                                        m_node_factory);
  ...
  MNM_IO_EV::build_od_factory_ev (m_file_folder, m_config, m_od_factory,
                                  m_node_factory);
  ...
  if (m_config->get_string ("routing_type") == "Hybrid")
    {
      ...
      Path_Table *_path_table
        = MNM_IO::load_path_table (m_file_folder + "/"
                                     + _tmp_conf->get_string ("path_file_name"),
                                   m_graph, _tmp_conf->get_int ("num_path"),
                                   _tmp_conf->get_string ("choice_portion")
                                     == "Buffer");
      ...
      OD_Candidate_POI_Table *_od_candidate_poi_table
        = MNM_IO_EV::load_candidate_poi_table (m_file_folder, m_config,
                                               m_od_factory, m_node_factory);

      m_routing
        = new MNM_Routing_Hybrid_EV (m_file_folder, m_graph, m_statistics,
                                     m_od_factory, m_node_factory,
                                     m_link_factory, _od_candidate_poi_table,
    ...
    {
      throw std::runtime_error (
        "MNM_Dta_EV::set_routing, routing_type must be Hybrid");
    }
  return 0;
}
  ...
  if (auto _routing = dynamic_cast<MNM_Routing_Hybrid_EV *> (m_routing))
    {
      ...
      Assert (
        _routing->m_routing_adaptive->m_od_candidate_poi_table != nullptr
        && !_routing->m_routing_adaptive->m_od_candidate_poi_table->empty ());
      _temp_flag = _routing->m_routing_adaptive
                     ->check_od_candidate_poi_table_connectivity ();
    }
  else
    {
...
MNM_Dta_EV::load_once (bool verbose, TInt load_int, TInt assign_int)
{
  ...
  for (auto _origin_it = m_od_factory->m_origin_map.begin ();
    ...
    {
      _origin = _origin_it->second;
      dynamic_cast<MNM_Origin_EV *> (_origin)
        ->adjust_multi_OD_seq_veh_routing_type (_ad_ratio);
      if (_releasing)
        {
          if (assign_int >= m_total_assign_inter)
          ...
          else
            {
              if ((m_config->get_string ("routing_type") == "Hybrid"))
                ...
                {
                  printf ("MNM_Dta_With_EV::load_once, routing type must be "
                          "Hybrid\n");
                }
            }
        }
  ...
  //     if (auto _charging_station =
  //     dynamic_cast<MNM_Charging_Station*>(_node)) {
