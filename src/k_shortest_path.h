...
public:
  ...
  ~KSP_Heap ();
  MNM_Cost *m_top_cost;
  heap<MNM_Cost *> m_binary_heap_cost
      = heap<MNM_Cost *> ([] (MNM_Cost *c) { return c->m_cost (); });
};
...
public:
  ...
  std::vector<MNM_Path *>
  build_path_with_except_last_sidetrack (MNM_Path *path);

  TInt m_dest_node_ID;
  std::unordered_map<TInt, TFlt> m_dist_to_dest_map;
  std::unordered_map<TInt, TInt> m_sp_tree_map;
  heap<KSP_Heap *> m_DG = heap<KSP_Heap *> (
      [] (KSP_Heap *th) { return th->m_top_cost->m_cost (); });
