...
public:
  MNM_Veh_Electrified (TInt ID, TInt start_time, TFlt starting_range = 100.,
                       bool using_roadside_charging = false,
                       TFlt full_range = 200.);
  ...
  TFlt m_full_range; // unit: miles, the miles the vehicle can travel with the
                     // full battery
  TFlt m_current_range; // unit: miles, the miles the vehicle can travel with
...
class MNM_Veh_Electrified_Delivery : public MNM_Veh_Electrified,
                                     public MNM_Veh_Delivery
{
public:
  MNM_Veh_Electrified_Delivery (TInt ID, TInt start_time,
                                TFlt starting_range = 100.,
                                bool using_roadside_charging = false,
                                TFlt full_range = 200.);
...
public:
  ...
  MNM_Veh_Electrified *
  make_veh_electrified (TInt timestamp, Vehicle_type veh_type,
                        bool using_roadside_charging = false);
  MNM_Veh_Electrified_Delivery *
  make_veh_electrified_delivery (TInt timestamp, Vehicle_type veh_type,
                                 bool using_roadside_charging = false);

  int set_ev_range (TFlt EV_starting_range_roadside_charging,
...
{
public:
  MNM_Origin_EV (TInt ID, TInt max_interval, TFlt flow_scalar, TInt frequency,
                 TInt ev_label, TFlt roadside_charging_ratio = 0.);
...
public:
  ...
  MNM_Origin *make_origin_ev (TInt ID, TInt max_interval, TFlt flow_scalar,
...
public:
  ...
  std::unordered_map<int, int>
    m_waiting_time_record; // time-dependent waiting time in loading intervals
};
...
typedef std::unordered_map<
  MNM_Origin *,
  std::unordered_map<MNM_Destination *, std::vector<MNM_Dnode *> *> *>
  OD_Candidate_POI_Table;
typedef std::unordered_map<MNM_Origin *,
                           std::unordered_map<MNM_Destination *, MNM_Dnode *> *>
  Best_POI_Table;
typedef std::unordered_map<MNM_Dnode *, std::unordered_map<TInt, TInt> *>
  Routing_Table2;
...
public:
  ...
  int
  set_shortest_path_tree (std::unordered_map<TInt, TInt> **shortest_path_tree,
                          MNM_Dnode *poi_node = nullptr,
                          MNM_Destination *veh_dest = nullptr);
...
public:
  static int
  build_od_factory_ev (const std::string &file_folder,
                       MNM_ConfReader *conf_reader, MNM_OD_Factory *od_factory,
                       MNM_Node_Factory *node_factory,
                       const std::string &file_name = "MNM_input_od");
  static int add_charging_station_node (const std::string &file_folder,
                                        MNM_ConfReader *conf_reader,
                                        MNM_Node_Factory *node_factory,
                                        const std::string &file_name
                                        = "MNM_input_charging_station");
  static OD_Candidate_POI_Table *load_candidate_poi_table (
  ...
  static int
  save_candidate_poi_table (const OD_Candidate_POI_Table &candidate_poi_table,
                            const std::string &file_name
                            = "MNM_input_candidate_charging_station");

  static int save_charging_station_record (const std::string &file_folder,
                                           MNM_Node_Factory *node_factory);
