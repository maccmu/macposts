...
MNM_Dlink_Multiclass::MNM_Dlink_Multiclass (
    ...
    TFlt ffs_truck)
    : MNM_Dlink::MNM_Dlink (
        ID, number_of_lane, length,
        0.0) // Note: m_ffs is not used in child class, so let it be 0.0
{
  m_ffs_car = ffs_car;
...
MNM_Dlink_Multiclass::~MNM_Dlink_Multiclass ()
{
  ...
  if (m_N_in_car != NULL)
    delete m_N_in_car;
  if (m_N_out_truck != NULL)
    delete m_N_out_truck;
  if (m_N_in_truck != NULL)
    delete m_N_in_truck;
  if (m_N_out_tree_car != NULL)
    delete m_N_out_tree_car;
  if (m_N_in_tree_car != NULL)
    delete m_N_in_tree_car;
  if (m_N_out_tree_truck != NULL)
    delete m_N_out_tree_truck;
  if (m_N_in_tree_truck != NULL)
                                                  ...
                                                  ffs_car, ffs_truck)
{
  ...
  TFlt _last_cell_length
      = m_length - TFlt (m_num_cells - 1) * _std_cell_length;
  ...
  for (int i = 0; i < number_tomove; ++i)
    {
      ...
      // update the vehicle position on current link. 0: at the begining,
      // 1: at the end.
...
int
MNM_Dlink_Ctm_Multiclass::init_cell_array (TFlt unit_time,
                                           ...
                                           TFlt last_cell_length)
{
  ...
  for (int i = 0; i < m_num_cells - 1; ++i)
    {
      cell = new Ctm_Cell_Multiclass (
          std_cell_length, unit_time,
          // Covert lane parameters to cell (link) parameters by
          // multiplying # of lanes
          ...
          TFlt (m_number_of_lane) * m_lane_flow_cap_truck, m_ffs_car,
  ...
  if (m_length > 0.0)
    {
      cell = new Ctm_Cell_Multiclass (
          ...
          unit_time, TFlt (m_number_of_lane) * m_lane_hold_cap_car,
          TFlt (m_number_of_lane) * m_lane_hold_cap_truck,
          TFlt (m_number_of_lane) * m_lane_critical_density_car,
          TFlt (m_number_of_lane) * m_lane_critical_density_truck,
          TFlt (m_number_of_lane) * m_lane_rho_1_N,
...
MNM_Dlink_Ctm_Multiclass::update_out_veh ()
{
      ...
      for (int i = 0; i < m_num_cells - 1; ++i)
        {
          ...
          _temp_out_flux_truck
              = m_cell_array[i]->m_space_fraction_truck
          ...
          // %.2f, %.2f, %.2f, %.2f, %d) ", 		_demand_car,
          // m_cell_array[i] -> m_cell_length, m_cell_array[i] ->
          // m_flow_cap_car, m_cell_array[i] -> m_ffs_car,
          // _supply_car, _temp_out_flux_car, m_cell_array[i] ->
          // m_space_fraction_car, m_cell_array[i] -> m_out_veh_car,
          // 		_demand_truck, _supply_truck, _temp_out_flux_truck,
          // m_cell_array[i] -> m_space_fraction_truck,
          // m_cell_array[i] -> m_out_veh_truck); 	printf("(%d, %d) ",
          // int(m_cell_array[i] -> m_out_veh_car),
          // int(m_cell_array[i] -> m_out_veh_truck));
          // }
        }
    }
  ...
  // int(m_cell_array[m_num_cells - 1] -> m_out_veh_car),
  // int(m_cell_array[m_num_cells - 1] -> m_out_veh_truck));
  return 0;
}
...
MNM_Dlink_Ctm_Multiclass::evolve (TInt timestamp)
{
  ...
  // 		for (int i = 0; i < m_num_cells - 1; ++i)
  // 		{
  // 			printf("(%d, %d) ", int(m_cell_array[i] ->
  // m_veh_queue_car.size()), int(m_cell_array[i] ->
  // m_veh_queue_truck.size()));
  // 		}
  // 	}
  // 	// m_class: 0 - private car, 1 - truck
  // 	for (_veh_it = m_finished_array.begin(); _veh_it !=
  ...
  // += 1;
  // 	}
  // 	printf("(%d, %d; %d, %d)\n", int(m_cell_array[m_num_cells - 1] ->
  // m_veh_queue_car.size()), 		int(m_cell_array[m_num_cells - 1] ->
  // m_veh_queue_truck.size()), int(_count_car), int(_count_truck));
  // }
      ...
      for (int i = 0; i < m_num_cells - 1; ++i)
        {
          ...
          // if (m_link_ID == _output_link)
          // 	printf("(%d, %d) ", int(m_cell_array[i] ->
          // m_volume_car), int(m_cell_array[i] -> m_volume_truck));
        }
    }
  ...
  // 	printf("(%d, %d; %d, %d)\n", int(m_cell_array[m_num_cells - 1] ->
  // m_veh_queue_car.size()), 		int(m_cell_array[m_num_cells - 1] ->
  // m_veh_queue_truck.size()), int(_count_car), int(_count_truck)); 	for
  // (_veh_it = m_finished_array.begin(); _veh_it != m_finished_array.end();
  // _veh_it++){ 		MNM_Veh_Multiclass *_veh =
  // dynamic_cast<MNM_Veh_Multiclass
  // *>(*_veh_it); 		printf("(%d-%d: %d->%d) ", int(_veh ->
  // m_veh_ID), int(_veh
  // -> m_class), int(_veh -> get_current_link() -> m_link_ID),
  ...
  for (int i = 0; i <= m_num_cells - 1; ++i)
    {
      ...
      _count_tot_vehs
          += m_cell_array[i]->m_volume_truck * m_veh_convert_factor;
    }
...
// 	TFlt _supply_car = m_cell_array[0] -> m_space_fraction_car *
// std::min(m_cell_array[0] -> m_flow_cap_car,
// TFlt(m_wave_speed_car * (m_cell_array[0] -> m_hold_cap_car - m_cell_array[0]
// -> m_perceived_density_car))); 	TFlt _supply_truck = m_cell_array[0] ->
// m_space_fraction_truck * std::min(m_cell_array[0] -> m_flow_cap_truck,
// 					TFlt(m_wave_speed_truck *
// (m_cell_array[0]
// -> m_hold_cap_truck - m_cell_array[0] -> m_perceived_density_truck)));
// TFlt _supply = std::max(TFlt(0.0), _supply_car) + m_veh_convert_factor *
// std::max(TFlt(0.0), _supply_truck);
...
MNM_Dlink_Ctm_Multiclass::get_link_supply ()
{
  ...
  double _tmp = std::min (
      double (m_cell_array[0]->m_flow_cap_car),
  ...
  for (size_t i = 0; i < _cur_size; ++i)
    {
      ...
      _veh->m_visual_position_on_link
          = float (1) / float (m_num_cells) / float (2);
    }
...
{
  // For adaptive routing and emissions, need modidication for multiclass
  // cases
  TFlt _cost, _spd;
      ...
      if (_rho <= _rhok)
      ...
      else
        {
          _spd = MNM_Ults::max (0.001 * m_ffs_car,
                                m_lane_flow_cap_car * (_rhoj - _rho)
                                    / (_rhoj - _rhok) / _rho);
        }
      _cost = m_length / _spd;
    }
...
MNM_Dlink_Ctm_Multiclass::Ctm_Cell_Multiclass::Ctm_Cell_Multiclass (
    ...
    TFlt flow_cap_car, TFlt flow_cap_truck, TFlt ffs_car, TFlt ffs_truck,
    TFlt wave_speed_car, TFlt wave_speed_truck, TFlt flow_scalar)
{
...
MNM_Dlink_Ctm_Multiclass::Ctm_Cell_Multiclass::update_perceived_density ()
{
  ...
  if (_density_car / m_critical_density_car
      ...
      <= 1)
    {
      ...
      // printf("-1, %.4f, %.4f", m_space_fraction_car,
      // m_space_fraction_truck);
    }
  // Semi-congested traffic (truck free-flow but car not)
               ...
               <= m_rho_1_N))
    {
      ...
      // printf("-2, %.4f, %.4f", m_space_fraction_car,
      // m_space_fraction_truck);
    }
  ...
  // m_perceived_density_truck > m_critical_density_truck
  else
    {
      ...
      if (m_volume_truck == 0)
        {
          ...
          // this case same speed (u) for both private cars and
          // trucks
          TFlt _u = (m_hold_cap_car - _density_car) * m_wave_speed_car
                    / _density_car;
          m_perceived_density_truck = (m_hold_cap_truck * m_wave_speed_truck)
                                      / (_u + m_wave_speed_truck);
        }
      // _density_car (m_volume_car) could still be 0 in some extreme
      // case
      ...
      m_space_fraction_car = _space_fraction_car;
      m_space_fraction_truck = _space_fraction_truck;
      // printf("-3, %.4f, %.4f", m_space_fraction_car,
      // m_space_fraction_truck);
    }
  // printf("\n");
  return 0;
  ...
  else
    {
      ...
      // case
      else if (m_volume_car == 0)
        {
          ...
          // trucks
          TFlt _u
              = (m_k_j_truck - _density_truck) * m_w_truck / _density_truck;
      ...
      else
        {
          ...
          _space_fraction_car
              = (_density_car * _density_truck * (m_w_car - m_w_truck)
                 + _tmp_2)
                / (_tmp_2 + _tmp_1);
          _space_fraction_truck
              = (_density_car * _density_truck * (m_w_truck - m_w_car)
                 + _tmp_1)
                / (_tmp_2 + _tmp_1);
          m_perceived_density_car = _density_car / _space_fraction_car;
          m_perceived_density_truck = _density_truck / _space_fraction_truck;
...
MNM_Dlink_Lq_Multiclass::evolve (TInt timestamp)
{
  ...
  TFlt _veh_to_move
      = _demand * m_flow_scalar - TInt (m_finished_array.size ());
  ...
  // TFlt _raw = _veh_to_move * (m_veh_convert_factor * _demand_truck /
  // _demand) / m_veh_convert_factor; TInt _tmp = std::floor(_raw); _raw =
  // _raw - TFlt(_tmp); TInt _tmp2 = 0; if (_raw < 0.2){ 	_tmp2 = 0;
  // }
  // else if (_raw > 0.8){
  // 	_tmp2 = 1;
  // }
...
MNM_Dlink_Lq_Multiclass::get_link_supply ()
{
  ...
  // TInt _count_truck = 0;
  // for (_veh_it = m_finished_array.begin(); _veh_it !=
  // m_finished_array.end(); _veh_it++){ 	MNM_Veh_Multiclass *_veh =
  // dynamic_cast<MNM_Veh_Multiclass *>(*_veh_it); 	if (_veh -> m_class ==
  // 0) _count_car += 1; 	if (_veh -> m_class == 1) _count_truck += 1;
  // }
  // m_volume_car = m_veh_queue_car.size() + _count_car;
  // m_volume_truck = m_veh_queue_truck.size() + _count_truck;
  // update_perceived_density();
  ...
  _supply_truck = std::max (
      _supply_truck, TFlt (m_space_fraction_truck * m_w_truck * 0.30
...
int
MNM_Dlink_Lq_Multiclass::clear_incoming_array ()
{
  MNM_Veh_Multiclass *_veh;
  size_t _cur_size = m_incoming_array.size ();
  for (size_t i = 0; i < _cur_size; ++i)
...
MNM_Dlink_Lq_Multiclass::get_link_tt ()
{
  ...
  if (_rho >= _rhoj)
    {
      _cost = MNM_Ults::max_link_cost (); // sean: i think we should use
                                          // rhoj, not rhok
    }
  else
    {
...
int
MNM_Dlink_Pq_Multiclass::clear_incoming_array ()
{
  MNM_Veh_Multiclass *_veh;
  ...
  while (!m_incoming_array.empty ())
    {
      if (_to_be_moved > 0)
        {
          _veh
              = dynamic_cast<MNM_Veh_Multiclass *> (m_incoming_array.front ());
          m_incoming_array.pop_front ();
...
MNM_Dlink_Pq_Multiclass::get_link_tt ()
{
  ...
  // 				m_lane_flow_cap * (_rhoj - _rho) / (_rhoj -
  // _rhok) / _rho);
  // 	}
  // 	_cost = m_length / _spd;
  // }
  ...
  for (unsigned i = 0; i < m_out_link_array.size (); ++i)
    {
      ...
      if (_next_link->m_N_in_car != NULL)
        {
          _next_link->m_N_in_car->add_increment (std::pair<TFlt, TFlt> (
              TFlt (timestamp + 1), TFlt (_moved_car) / m_flow_scalar));
        }
      if (_next_link->m_N_in_truck != NULL)
        {
          _next_link->m_N_in_truck->add_increment (std::pair<TFlt, TFlt> (
              TFlt (timestamp + 1), TFlt (_moved_truck) / m_flow_scalar));
        }
  ...
  for (size_t i = 0; i < m_in_link_array.size (); ++i)
    {
      ...
      if (_from_link->m_N_out_car != NULL)
        {
          _from_link->m_N_out_car->add_increment (std::pair<TFlt, TFlt> (
              TFlt (timestamp + 1), TFlt (_moved_car) / m_flow_scalar));
        }
      if (_from_link->m_N_out_truck != NULL)
        {
          _from_link->m_N_out_truck->add_increment (std::pair<TFlt, TFlt> (
              TFlt (timestamp + 1), TFlt (_moved_truck) / m_flow_scalar));
        }
    }
  return 0;
      ...
      for (size_t i = 0; i < m_in_link_array.size (); ++i)
        {
          ...
          // m_link_ID), int(_out_link -> m_link_ID),
          // double(_to_move));
          ...
          // 		MNM_Veh_Multiclass *_veh2 =
          // dynamic_cast<MNM_Veh_Multiclass *>(*_veh_it2);
                      ...
                      if (_to_move < _equiv_num)
                        {
                          ...
                          // or not in this case base
                          // on the probability =
                          // _to_move/_equiv_num < 1
                          // Will result in WRONG
                          // INCOMING ARRAY SIZE if
                              ...
                              if (_veh->m_class == 0)
                                {
                                  ...
                                  if (_ilink->m_N_in_tree_car != NULL)
                                    {
                                      _ilink->m_N_in_tree_car->add_flow (
                                          TFlt (timestamp), 1 / m_flow_scalar,
                                          _veh->m_path,
                              ...
                              else
                                {
                                  ...
                                  if (_ilink->m_N_in_tree_truck != NULL)
                                    {
                                      _ilink->m_N_in_tree_truck->add_flow (
                                          TFlt (timestamp), 1 / m_flow_scalar,
                                          _veh->m_path,
                                          _veh->m_assign_interval);
                                    }
          ...
          if (_to_move > 0.001)
            {
              printf ("Something wrong during the vehicle "
                      ...
                      (float)_to_move);
              // printf("The finished veh queue is now size
  ...
  for (size_t j = 0; j < m_out_link_array.size (); ++j)
    {
      ...
      if (_out_link->m_N_out_car != NULL)
        {
          _out_link->m_N_in_car->add_increment (std::pair<TFlt, TFlt> (
              TFlt (timestamp + 1), TFlt (_temp_sum_car) / m_flow_scalar));
        }
      if (_out_link->m_N_out_truck != NULL)
        {
          _out_link->m_N_in_truck->add_increment (std::pair<TFlt, TFlt> (
              TFlt (timestamp + 1), TFlt (_temp_sum_truck) / m_flow_scalar));
        }
    }
  ...
  for (size_t i = 0; i < m_in_link_array.size (); ++i)
    {
      ...
      for (size_t j = 0; j < m_out_link_array.size (); ++j)
        {
          _out_link
              = dynamic_cast<MNM_Dlink_Multiclass *> (m_out_link_array[j]);
          _temp_sum_car += m_veh_moved_car[i * _offset + j];
          _temp_sum_truck += m_veh_moved_truck[i * _offset + j];
        }
      if (_in_link->m_N_in_car != NULL)
        {
          _in_link->m_N_out_car->add_increment (std::pair<TFlt, TFlt> (
              TFlt (timestamp + 1), TFlt (_temp_sum_car) / m_flow_scalar));
        }
      if (_in_link->m_N_in_truck != NULL)
        {
          _in_link->m_N_out_truck->add_increment (std::pair<TFlt, TFlt> (
              TFlt (timestamp + 1), TFlt (_temp_sum_truck) / m_flow_scalar));
        }
    }
...
MNM_Dnode_FWJ_Multiclass::MNM_Dnode_FWJ_Multiclass (TInt ID, TFlt flow_scalar,
                                                    TFlt veh_convert_factor)
    : MNM_Dnode_Inout_Multiclass::MNM_Dnode_Inout_Multiclass (
        ID, flow_scalar, veh_convert_factor)
{
}
      ...
      for (int i = 0; i < _veh_to_release; ++i)
        {
          if (adaptive_ratio == TFlt (0))
            {
              _veh = _vfactory->make_veh_multiclass (
                  current_interval, MNM_TYPE_STATIC, TInt (0));
            }
          else if (adaptive_ratio == TFlt (1))
            {
              _veh = _vfactory->make_veh_multiclass (
                  current_interval, MNM_TYPE_ADAPTIVE, TInt (0));
            }
          else
            {
      ...
      for (int i = 0; i < _veh_to_release; ++i)
        {
          if (adaptive_ratio == TFlt (0))
            {
              _veh = _vfactory->make_veh_multiclass (
                  current_interval, MNM_TYPE_STATIC, TInt (1));
            }
          else if (adaptive_ratio == TFlt (1))
            {
              _veh = _vfactory->make_veh_multiclass (
                  current_interval, MNM_TYPE_ADAPTIVE, TInt (1));
            }
          else
            {
              TFlt _r = MNM_Ults::rand_flt ();
    ...
    TFlt veh_convert_factor, TFlt flow_scalar)
{
  ...
  switch (link_type)
    ...
    case MNM_TYPE_CTM_MULTICLASS:
      _link = new MNM_Dlink_Ctm_Multiclass (
          ID, number_of_lane, length, lane_hold_cap_car, lane_hold_cap_truck,
          lane_flow_cap_car, lane_flow_cap_truck, ffs_car, ffs_truck,
          unit_time, veh_convert_factor, flow_scalar);
      break;
    case MNM_TYPE_LQ_MULTICLASS:
      _link = new MNM_Dlink_Lq_Multiclass (
          ID, number_of_lane, length, lane_hold_cap_car, lane_hold_cap_truck,
          lane_flow_cap_car, lane_flow_cap_truck, ffs_car, ffs_truck,
          unit_time, veh_convert_factor, flow_scalar);
      break;
    case MNM_TYPE_PQ_MULTICLASS:
      _link = new MNM_Dlink_Pq_Multiclass (
          ID, number_of_lane, length, lane_hold_cap_car, lane_hold_cap_truck,
          lane_flow_cap_car, lane_flow_cap_truck, ffs_car, ffs_truck,
          unit_time, veh_convert_factor, flow_scalar);
      break;
          ...
          if (_words.size () == 3)
            {
              ...
              if (_type == "FWJ")
                {
                  _node_factory->make_node_multiclass (
                      _node_ID, MNM_TYPE_FWJ_MULTICLASS, _flow_scalar,
                      _veh_convert_factor);
                  continue;
                }
              if (_type == "DMOND")
                {
                  _node_factory->make_node_multiclass (
                      _node_ID, MNM_TYPE_ORIGIN_MULTICLASS, _flow_scalar,
                      _veh_convert_factor);
                  continue;
                }
              if (_type == "DMDND")
                {
                  _node_factory->make_node_multiclass (
                      _node_ID, MNM_TYPE_DEST_MULTICLASS, _flow_scalar,
                      _veh_convert_factor);
                  continue;
                }
...
MNM_IO_Multiclass::build_link_factory_multiclass (
    std::string file_folder, MNM_ConfReader *conf_reader,
    MNM_Link_Factory *link_factory, std::string file_name)
{
  /* find file */
          ...
          if (_words.size () == 11)
            {
              ...
              if (_type == "PQ")
                {
                  _link_factory->make_link_multiclass (
                      _link_ID, MNM_TYPE_PQ_MULTICLASS, _number_of_lane,
                      _length, _lane_hold_cap_car, _lane_hold_cap_truck,
                      _lane_flow_cap_car, _lane_flow_cap_truck, _ffs_car,
                      _ffs_truck, _unit_time, _veh_convert_factor,
                      _flow_scalar);
                  continue;
                }
              if (_type == "LQ")
                {
                  _link_factory->make_link_multiclass (
                      _link_ID, MNM_TYPE_LQ_MULTICLASS, _number_of_lane,
                      _length, _lane_hold_cap_car, _lane_hold_cap_truck,
                      _lane_flow_cap_car, _lane_flow_cap_truck, _ffs_car,
                      _ffs_truck, _unit_time, _veh_convert_factor,
                      _flow_scalar);
              ...
              if (_type == "CTM")
                {
                  _link_factory->make_link_multiclass (
                      _link_ID, MNM_TYPE_CTM_MULTICLASS, _number_of_lane,
                      _length, _lane_hold_cap_car, _lane_hold_cap_truck,
                      _lane_flow_cap_car, _lane_flow_cap_truck, _ffs_car,
                      _ffs_truck, _unit_time, _veh_convert_factor,
                      _flow_scalar);
...
int
MNM_IO_Multiclass::build_demand_multiclass (std::string file_folder,
...
{
  if (m_veh_factory != NULL)
    delete m_veh_factory;
  if (m_node_factory != NULL)
    delete m_node_factory;
  if (m_link_factory != NULL)
    delete m_link_factory;
  if (m_od_factory != NULL)
    delete m_od_factory;
  if (m_config != NULL)
    delete m_config;
  ...
  // change assign_freq to 12 (1 minute) and total_assign_interval to
  // max_interval*_num_of_minute
...
TFlt
get_link_inflow_car (MNM_Dlink_Multiclass *link, TFlt start_time,
                     TFlt end_time)
{
  if (link == NULL)
    {
  ...
  if (link->m_N_in_car == NULL)
    {
      throw std::runtime_error ("Error, get_link_inflow_car link "
...
TFlt
get_link_inflow_car (MNM_Dlink_Multiclass *link, TInt start_time,
                     TInt end_time)
{
  if (link == NULL)
    {
                       ...
                       TFlt end_time)
{
  ...
  if (link->m_N_in_truck == NULL)
    {
      throw std::runtime_error ("Error, get_link_inflow_truck link "
                                "cummulative curve is not installed");
    }
...
get_average_waiting_time_at_intersection (MNM_Dlink_Multiclass *link)
{
  if (link == NULL)
    {
      throw std::runtime_error (
          "Error, get_average_waiting_time_at_intersection link is "
          "null");
    }
  if (link->m_N_in_car == NULL)
    {
  ...
  if (link->m_N_in_truck == NULL)
    {
      throw std::runtime_error (
          "Error, get_average_waiting_time_at_intersection link truck "
          "in cummulative curve is not installed");
    }
  TFlt _tot_vehs = 0;
...
get_travel_time_car (MNM_Dlink_Multiclass *link, TFlt start_time)
{
  ...
  if (link->m_N_in_car == NULL)
    {
      throw std::runtime_error ("Error, get_travel_time_car link "
                                "cummulative curve is not installed");
    }
  ...
  // get the earliest time point in m_N_in_car that reach the inflow ==
  // _cc_flow as the true stat_time
  TFlt _true_start_time = link->m_N_in_car->get_time (_cc_flow);

  // get the earliest time point in m_N_out_car that reach the outflow ==
  // _cc_flow as the end_time
...
get_travel_time_truck (MNM_Dlink_Multiclass *link, TFlt start_time)
{
  ...
  if (link->m_N_in_truck == NULL)
    {
      throw std::runtime_error ("Error, get_travel_time_truck link "
                                "cummulative curve is not installed");
    }
  // printf("%.2f\n", start_time);
  ...
  // get the earliest time point in m_N_in_truck that reach the inflow ==
  // _cc_flow as the true stat_time
  TFlt _true_start_time = link->m_N_in_truck->get_time (_cc_flow);

  // get the earliest time point in m_N_out_truck that reach the outflow ==
  // _cc_flow as the end_time
                     ...
                     TFlt start_time, TFlt end_time)
{
  ...
  if (link->m_N_in_tree_car == NULL)
    {
      throw std::runtime_error (
          "Error, add_dar_records_car link cummulative curve tree is "
          "not installed");
    }
  MNM_Path *_path;
                       ...
                       TFlt end_time)
{
  ...
  if (link->m_N_in_tree_truck == NULL)
    {
      throw std::runtime_error (
          "Error, add_dar_records_truck link cummulative curve tree is "
          "not installed");
    }
  MNM_Path *_path;
...
int
MNM_Cumulative_Emission_Multiclass::update ()
{
  ...
  for (MNM_Dlink *link : m_link_vector)
    {
      ...
      m_NOX_truck += calculate_NOX_rate_truck (_v_converted)
                     ...
                     * _mlink->get_link_flow_truck ();
      m_VMT_truck
          += (_v * m_unit_time / TFlt (1600)) * _mlink->get_link_flow_truck ();
...
MNM_Cumulative_Emission_Multiclass::output ()
{
  ...
  printf ("fuel: %lf gallons, CO2: %lf g, HC: %lf g, CO: %lf g, NOX: %lf g, "
          ...
          m_NOX_truck (), m_VMT_truck (), m_VHT_truck ());
  return 0;
}