...
int
MNM_DMOND::evolve (TInt timestamp)
{
              ...
              if (_to_link == _link)
                {
                  ...
                  _que_it = m_in_veh_queue.erase (
                    _que_it); // c++ 11, erase during loop,
...
int
MNM_DMOND::add_out_link (MNM_Dlink *out_link)
{
...
void
MNM_DMOND::print_info ()
{
  ;
}

int
MNM_DMOND::hook_up_origin (MNM_Origin *origin)
{
  m_origin = origin;
  return 0;
}
...
MNM_DMDND::~MNM_DMDND () { m_out_veh_queue.clear (); }

int
MNM_DMDND::add_in_link (MNM_Dlink *link)
{
...
int
MNM_DMDND::evolve (TInt timestamp)
{
...
void
MNM_DMDND::print_info ()
{
  ;
}

int
MNM_DMDND::hook_up_destination (MNM_Destination *dest)
{
  m_dest = dest;
  return 0;
}
...
int
MNM_Dnode_Inout::prepare_supplyANDdemand ()
{
      ...
      for (size_t j = 0; j < m_out_link_array.size (); ++j)
        {
          ...
          for (_veh_it = _in_link->m_finished_array.begin ();
               _veh_it != _in_link->m_finished_array.end (); _veh_it++)
            {
              if ((*_veh_it)->get_next_link () == _out_link)
                _count += 1;
            }
      ...
      for (size_t i = 0; i < m_in_link_array.size (); ++i)
        {
          ...
          // _offset + j] * m_flow_scalar, m_veh_tomove[i * _offset + j]);
        }
...
int
MNM_Dnode_Inout::record_cumulative_curve (TInt timestamp)
{
  TInt _num_to_move;
  TInt _temp_sum;
  MNM_Dlink *_in_link, *_out_link;
  ...
  for (size_t j = 0; j < m_out_link_array.size (); ++j)
    {
      ...
      // make the queue randomly perturbed, may not be true in signal controlled
      // intersection, violate FIFO random_shuffle(_out_link ->
      // m_incoming_array.begin(), _out_link -> m_incoming_array.end());
    }
...
void
MNM_Dnode_Inout::print_info ()
{
  ;
}

int
MNM_Dnode_Inout::add_out_link (MNM_Dlink *out_link)
{
  m_out_link_array.push_back (out_link);
  return 0;
}
...
int
MNM_Dnode_Inout::add_in_link (MNM_Dlink *in_link)
{
...
int
MNM_Dnode_Inout::evolve (TInt timestamp)
{
...
void
MNM_Dnode_FWJ::print_info ()
{
  ;
}

int
MNM_Dnode_FWJ::compute_flow ()
{
...
void
MNM_Dnode_GRJ::print_info ()
{
  ;
}

int
MNM_Dnode_GRJ::compute_flow ()
{
...
std::vector<int>
MNM_Dnode_GRJ::getOnLocations (int a)
{
      ...
      for (std::vector<MNM_Dlink *> v : m_pow)
        {
          ...
          for (size_t i = 0; i < m_in_link_array.size (); ++i)
            {
              if (std::find (v.begin (), v.end (), m_in_link_array[i])
                  != v.end ())
                {
                  _down += m_C_a[i]
                           * MNM_Ults::divide (m_demand[i * _offset + j],
                                               m_supply[j]);
                }
              else
                {
          ...
          // printf("for j is %d, the value of cal is %lf, s is %lf, up is %lf,
          // down is %lf\n", j, (m_supply[j] - _up)/_down ,m_supply[j](), _up(),
          // _down());
