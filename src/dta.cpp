...
MNM_Dta::set_routing ()
{
  if (m_config->get_string ("routing_type") == "Adaptive")
    ...
    {
      // Path_Table *_path_table = MNM::build_pathset(m_graph,
      // m_od_factory, m_link_factory);
  ...
  // Note here still only one path_table and buffer, but in buffer file each
  // row contains: Row #k : [probabilities choosing route k in all intervals
  // for cars] [probabilities choosing route k in all intervals for trucks]
  // For Bi-class Fixed routing, just set both adaptive_ratio_car=0 &
...
MNM_Dta::is_ok ()
{
  ...
  // For each node in the graph check that its neighbors are also nodes in
  // the graph.
...
MNM_Dta::pre_loading ()
{
  ...
  // MNM_Dlink *_link;
  // for (auto _link_it = m_link_factory -> m_link_map.begin(); _link_it !=
  // m_link_factory -> m_link_map.end(); _link_it++){
...
MNM_Dta::load_once (bool verbose, TInt load_int, TInt assign_int)
{
  ...
  for (auto _link_it = m_link_factory->m_link_map.begin ();
       _link_it != m_link_factory->m_link_map.end (); _link_it++)
    {
      ...
      //   printf("Current Link %d:, traffic flow %.4f, incomming %d,
      //   finished %d\n",
      //       _link -> m_link_ID(), _link -> get_link_flow()(),
      //       (int)_link -> m_incoming_array.size(),  (int)_link ->
  ...
  while (!finished_loading (_cur_int))
    {
      ...
      if (_cur_int % m_assign_freq == 0 || _cur_int == 0)
        {
          for (auto _origin_it = m_od_factory->m_origin_map.begin ();
               _origin_it != m_od_factory->m_origin_map.end (); _origin_it++)
            {
              ...
              // _assign_inter = _assign_inter %
              // m_total_assign_inter; _origin ->
              // release_one_interval(_cur_int, m_veh_factory,
              // _assign_inter, TFlt(0));
            }
          _assign_inter += 1;
        }
      ...
      // record queuing vehicles after node evolve, which is num of
      // vehicles in finished array
      record_queue_vehicles ();
      ...
      for (auto _link_it = m_link_factory->m_link_map.begin ();
           _link_it != m_link_factory->m_link_map.end (); _link_it++)
        {
          _link = _link_it->second;
          // if (_link -> get_link_flow() >= 0){
          //   printf("Current Link %d:, traffic flow %.4f, incomming
          //   %d, finished %d\n",
          //       _link -> m_link_ID(), _link -> get_link_flow()(),
          //       (int)_link -> m_incoming_array.size(),  (int)_link
          //       -> m_finished_array.size());
  ...
  for (auto _map_it : m_link_factory->m_link_map)
    {
      ...
      if (_map_it.second->m_ffs == 0.0)
        _tot_queue_size += _queue_size; // PQ not included
...
print_vehicle_info (MNM_Veh_Factory *veh_factory)
{
  ...
  for (auto _map_it : veh_factory->m_veh_map)
    {
      _veh = _map_it.second;
      printf ("Vehicle ID %d, from origin node %d to destination node "
              "%d, is current on link %d and heads to link %d\n",
              _veh->m_veh_ID (),
