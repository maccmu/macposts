...
MNM_Pre_Routing::MNM_Pre_Routing (Path_Table *path_table,
              ...
              if (routing_table->find (_origin_node_ID)
                  == routing_table->end ())
                {
                  ...
                  for (int _k = 1; _k < _path_size; _k++)
                    {
                      TFlt *demand_copy = new TFlt[size_demand];
                      for (int demand_it = 0; demand_it < size_demand;
                           demand_it++)
                        {
                                   ...
                                   TFlt lambda)
{
  ...
  // std::cout <<  "pre routing size: " << routing_table
  // ->at(oid).at(did).size() <<std::endl;
  for (auto _path_demand = routing_table->at (oid).at (did).begin ();
       _path_demand != routing_table->at (oid).at (did).end (); _path_demand++)
    {
...
std::string
MNM_Pre_Routing::toString ()
{
  std::string result = "Pre routing table\n";
  MNM_Path *_path;
  for (auto _oit = routing_table->begin (); _oit != routing_table->end ();
       _oit++)
    {
      TInt _oid = _oit->first;
      result += "From Origin " + std::to_string (_oid);
      for (auto _dit = _oit->second.begin (); _dit != _oit->second.end ();
           _dit++)
        {
          ...
          for (auto _pit = _dit->second.begin (); _pit != _dit->second.end ();
               _pit++)
            {
...
int
MNM_Pre_Routing::update_demand (MNM_OD_Factory *od_factory)
{
