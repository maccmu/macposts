...
public:
  ...
  //std::vector<Bus_Boarding_Alighting_Record> m_boarding_alighting_records;
  ...
  py::array_t<double> get_path_tt_car (py::array_t<int> link_IDs,
                                       py::array_t<double> start_intervals);
  py::array_t<double> get_bus_boarding_alighting_record ();
  py::tuple get_sparse_dar_matrix_pnr_by_round();
  py::tuple get_sparse_dar_matrix_bt_by_round();
...
init (py::module &m)
{
  py::class_<Mmdta> (m, "Mmdta")
    ...
    .def("get_bus_boarding_alighting_record", &Mmdta::get_bus_boarding_alighting_record)
    .def("get_sparse_dar_matrix_pnr_by_round", &Mmdta::get_sparse_dar_matrix_pnr_by_round)
    .def("get_sparse_dar_matrix_bt_by_round", &Mmdta::get_sparse_dar_matrix_bt_by_round)
...
py::tuple Mmdta::get_sparse_dar_matrix_bt_by_round()
{
    ...
    std::map<std::tuple<TInt, TInt, std::string>, int> row_mapping;
    ...
    for (const auto& r : row_reverse)
        row_names.append(py::make_tuple(std::get<0>(r), std::get<1>(r), std::get<2>(r)));

    py::list col_names;
    for (const auto& c : col_reverse)
        col_names.append(py::make_tuple(c.first, c.second));

    return py::make_tuple(
        py::array_t<int>(row_idx_vec.size(), row_idx_vec.data()),
        py::array_t<int>(col_idx_vec.size(), col_idx_vec.data()),
        py::array_t<double>(val_vec.size(), val_vec.data()),
        row_names,
        col_names
    );
}
      ...
      for (size_t i = 0; i < m_link_vec_driving.size (); ++i)
        {
          ...
          result_ptr[i * l + t] = (((m_link_vec_driving[i]->m_length) / _tt) > m_link_vec_driving[i]->m_ffs_car ? 
                                     m_link_vec_driving[i]->m_ffs_car : ((m_link_vec_driving[i]->m_length) / _tt)) * 3600. / 1600.; // mile per hour
        }
    }
  return result;
}
          ...
          double _tt
            = MNM_DTA_GRADIENT::
                get_travel_time_truck (m_link_vec_driving[i],
                                       ...
                                       m_mmdta->m_current_loading_interval)
              * m_mmdta->m_unit_time; // seconds
          result_ptr[i * l + t] = (((m_link_vec_driving[i]->m_length) / _tt) > m_link_vec_driving[i]->m_ffs_truck ? 
                                     m_link_vec_driving[i]->m_ffs_truck : ((m_link_vec_driving[i]->m_length) / _tt)) * 3600. / 1600.; // mile per hour
        }
    }
  return result;
}
      ...
      for (size_t i = 0; i < m_link_vec_bus.size (); ++i)
        {
          ...
          if (std::isinf (_tt))
            ...
            {
              // find max truck free flow speed
              auto _comp_spd = [](MNM_Dlink *l1, MNM_Dlink *l2)
                   {
                       return dynamic_cast<MNM_Dlink_Multiclass*>(l1) -> m_ffs_truck < dynamic_cast<MNM_Dlink_Multiclass*>(l2) -> m_ffs_truck;
                   };
              auto _r = std::max_element(m_link_vec_bus[i]-> m_overlapped_driving_link_vec.begin(), m_link_vec_bus[i]-> m_overlapped_driving_link_vec.end(), _comp_spd);
              auto _max_spd = dynamic_cast<MNM_Dlink_Multiclass*>(*_r) -> m_ffs_truck;
              result_ptr[i * l + t] = (((m_link_vec_bus[i]-> m_length) / _tt) > _max_spd ? 
                                         _max_spd : ((m_link_vec_bus[i]->m_length) / _tt)) * 3600. / 1600.; // mile per hour
            }
        }
    }
  return result;
