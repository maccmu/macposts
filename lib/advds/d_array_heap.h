...
public:
  ...
  public:
    ...
    {
      if (nfilled == 0)
        return nullptr; // no children, no problem

      return *std::min_element (
          children, children + nfilled,
          [] (node *a, node *b) { return a->cost < b->cost; });
    }

    node (const T &value, double cost)
        : value (value), cost (cost), parent (nullptr), nfilled (0){};
    node (const T &value, double cost, node *parent)
        : value (value), cost (cost), parent (parent), nfilled (0){};
    ...
    node *
    next_sibling (const node *np) const
    {
      assert (
          nfilled
          == n); // this function should be called on fully filled nodes only
      if (children[n - 1] == np)
        {
    ...
    next_with_free (const node *me)
    {
      ...
      return with_free; // might be nullptr if we started from the lowest
                        // rightmost node
    };
  ...
  heap (std::function<double (T)> cost_fn)
      : cost_fn (cost_fn), root (nullptr){};
