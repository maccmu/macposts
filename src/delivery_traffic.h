...
public:
    ...
    int set_multi_od_seq(std::vector<std::pair<MNM_Origin*, MNM_Destination*>>* multi_od_seq);
    ...
    // intervals, used to consider some delays, e.g., delivery time or pickup waiting time at origins for mobility service vehicles in multimodal
...
public:
    ...
    virtual MNM_Dlink *make_link (TInt ID, DLink_type link_type, TFlt lane_hold_cap,
                                ...
                                TFlt ffs, TFlt unit_time, TFlt flow_scalar) override;
};
...
{
public:
    MNM_Origin_Delivery(TInt ID, TInt max_interval, TFlt flow_scalar, TInt frequency);
    virtual ~MNM_Origin_Delivery();

    int add_multi_OD_seq_demand(std::vector<std::pair<MNM_Origin*, MNM_Destination*>> *multi_od_seq, TFlt* demand);
    ...
    std::unordered_map<std::vector<std::pair<MNM_Origin*, MNM_Destination*>>*, TFlt*> m_demand_multi_OD_seq;
...
public:
  ...
  virtual MNM_Origin* make_origin(TInt ID, TInt max_interval, TFlt flow_scalar, TInt frequency) override;
};
...
{
public:
  static int build_od_factory_delivery(const std::string& file_folder, MNM_ConfReader *conf_reader,
  ...
  static int build_demand_multi_OD_seq(const std::string& file_folder, MNM_ConfReader *conf_reader,
                                  MNM_OD_Factory *od_factory, 
                                  const std::string& od_file_name = "MNM_input_multi_od_seq",
                                  const std::string& demand_file_name = "MNM_input_multi_od_seq_demand");
};
