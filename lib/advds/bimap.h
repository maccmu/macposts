...
#define BIMAP_OFFSETOF_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761(type, member)     \
  (__extension__({                                                            \
    type *t = 0;                                                              \
    reinterpret_cast<size_t> (reinterpret_cast<const char *> (&(t->member))); \
  }))
#else
#define BIMAP_OFFSETOF_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761(type, member)     \
...
struct equal_types_no
{
  char m[2];
};
equal_types_no equal_types_helper (...);
template <typename T> equal_types_yes equal_types_helper (T, T);
template <typename T> struct equal_types_ptr
{
  static const T *make ();
};
...
template <typename T, typename U> struct equal_types
{
  enum
  {
    value = (sizeof (equal_types_helper (equal_types_ptr<T>::make (),
                                         equal_types_ptr<U>::make ()))
...
template <bool test, typename then, typename els> struct select
{
 ...
 * sense that an inv_pair<T,U> can be reinterpret_cast'ed to be an
 * std::pair<U,T>. To preserve symmetry, std::pair's should provide the
 * corresponding casting operator to inv_pair. As this is not feasible
 * (predefined classes cannot be injected this type of operators), a derivation
 * of std::pair called direct_pair is defined that plays the role of std::pair.
 */
...
template <typename first_type, typename second_type> struct direct_pair;

template <typename first_type_, typename second_type_> struct inv_pair
{
  ...
  {
    return *reinterpret_cast<const direct_pair<second_type, first_type> *> (
        this);
  }
};
...
template <typename first_type, typename second_type>
inv_pair<first_type, second_type>
make_inv_pair (const first_type &first, const second_type &second)
{
...
template <typename first_type, typename second_type>
bool
operator== (const inv_pair<first_type, second_type> &x,
...
template <typename first_type, typename second_type>
bool
operator== (const std::pair<first_type, second_type> &x,
            const inv_pair<first_type, second_type> &y)
{
  return x.first == y.first && x.second == y.second;
}
...
protected:
  ...
  template <typename bimap_type> class from_binding
  {
  public:
    ...
    const typename bimap_type::to_type &
    get () const
    {
    ...
    operator const typename bimap_type::to_type & () const { return get (); }

    from_binding &
    operator= (const typename bimap_type::to_type &t)
    {
      ...
      if (tit != bm.tset.end ())
        { /* v.second shouldn't be already in */
          /* small chance the pair (f,t) is already inserted */
          if (fit != bm.fset.end ()
              && bimap_type::element_by_from (*fit)
                     == bimap_type::element_by_to (*tit))
            {
              return *this;
            }
          else
  ...
  template <typename bimap_type> class const_from_binding
  {
  public:
    const_from_binding (const bimap_type &bm,
                        const typename bimap_type::from_type &f)
        : bm (bm), f (f)
    {
    }
  ...
  template <typename bimap_type> class to_binding
  {
  public:
    ...
    const typename bimap_type::from_type &
    get () const
    {
    ...
    operator const typename bimap_type::from_type & () const { return get (); }

    to_binding &
    operator= (const typename bimap_type::from_type &f)
    {
      /* MSVC++ chokes if a ctor for typename bimap_type::element
       * is called directly.
       */
  ...
  template <typename bimap_type> class const_to_binding
  {
  public:
    const_to_binding (const bimap_type &bm,
                      const typename bimap_type::to_type &t)
        : bm (bm), t (t)
    {
    }
...
template <typename from_type_, typename to_type_, typename from_compare,
          typename to_compare, typename allocator_type_>
...
private:
  ...
  struct p_compare /* pointer compare based on value compare */
  {
    ...
    bool
    operator() (const type *p1, const type *p2) const
    {
      return c (*p1, *p2);
    }
    compare
  ...
  typedef std::set<const from_type *, p_compare<from_type, from_compare>,
                   BIMAP_REBIND_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761 (
                       allocator_type_, const from_type *)>
      from_set;
  typedef std::set<const to_type *, p_compare<to_type, to_compare>,
                   BIMAP_REBIND_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761 (
                       allocator_type_, const to_type *)>
      to_set;
  ...
  typedef direct_pair<const from_type, const to_type> element;
  ...
  static element *
  element_by_from (const from_type *pf)
  {
    return reinterpret_cast<element *> (
        reinterpret_cast<char *> (const_cast<from_type *> (pf))
        - BIMAP_OFFSETOF_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761 (element,
                                                               first));
  }

  static element *
  element_by_to (const to_type *pt)
  {
    return reinterpret_cast<element *> (
        reinterpret_cast<char *> (const_cast<to_type *> (pt))
        - BIMAP_OFFSETOF_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761 (element,
                                                               second));
  }
...
public:
  ...
  public:
    ...
    bool
    operator== (const from_impl &r) const
    {
    ...
    public:
      ...
      {
        if (kcomp (x.first, y.first))
          return true;
        if (kcomp (y.first, x.first))
          return false;
                                          ...
                                          const value_type>
    {
      ...
      friend class prebimap<from_type, to_type, from_compare, to_compare,
                            allocator_type>;
    ...
    typedef std::reverse_bidirectional_iterator<
        const_iterator, const value_type, const_reference, const value_type *,
        difference_type>
        reverse_iterator;
#else
    ...
    begin ()
    {
      return iterator (owner ().fset.begin ());
    }
    const_iterator
    begin () const
    {
    ...
    {
      return iterator (owner ().fset.end ());
    }
    const_iterator
    end () const
    {
    ...
    from_binding<prebimap<from_type_, to_type_, from_compare, to_compare,
                          allocator_type_> >
    operator[] (const from_type_ &f)
    {
      return from_binding<prebimap<from_type_, to_type_, from_compare,
                                   to_compare, allocator_type_> > (owner (),
                                                                   f);
    }

    const_from_binding<prebimap<from_type_, to_type_, from_compare, to_compare,
                                allocator_type_> >
    operator[] (const from_type_ &f) const
    {
      return const_from_binding<prebimap<from_type_, to_type_, from_compare,
    ...
    void
    insert (it_type first, it_type last)
    {
      while (first != last)
        {
          insert (*first);
    ...
    value_comp () const
    {
      return value_compare (owner ().fset.key_comp ().get_compare (),
  ...
  protected:
    ...
    owner ()
    {
      return *reinterpret_cast<prebimap *> (
          ...
          - BIMAP_OFFSETOF_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761 (prebimap,
                                                                 from));
    };
    const prebimap &
    owner () const
    ...
    bool
    adjacent (const_fset_iterator fit, const from_type_ &f) const
    {
      if (fit == owner ().fset.end ())
        {
          if (owner ().fset.size () == 0)
            return true;
  ...
  {
    friend class prebimap<from_type_, to_type_, from_compare, to_compare,
                          allocator_type_>;
  ...
  public:
    ...
    bool
    operator== (const to_impl &r) const
    {
      return size () == r.size () && std::equal (begin (), end (), r.begin ());
    }
    ...
    begin ()
    {
      return iterator (owner ().tset.begin ());
    }
    const_iterator
    begin () const
    {
    ...
    {
      return iterator (owner ().tset.end ());
    }
    const_iterator
    end () const
    {
    ...
    to_binding<prebimap<from_type_, to_type_, from_compare, to_compare,
                        allocator_type_> >
    operator[] (const to_type_ &t)
    {
      return to_binding<prebimap<from_type_, to_type_, from_compare,
                                 to_compare, allocator_type_> > (owner (), t);
    }

    const_to_binding<prebimap<from_type_, to_type_, from_compare, to_compare,
                              allocator_type_> >
    operator[] (const to_type_ &t) const
    {
      return const_to_binding<prebimap<from_type_, to_type_, from_compare,
                                       to_compare, allocator_type_> > (
          owner (), t);
    }
    ...
    value_comp () const
    {
      return value_compare (owner ().tset.key_comp ().get_compare (),
  ...
  protected:
    ...
    bool
    adjacent (const_tset_iterator tit, const to_type_ &t) const
    {
      if (tit == owner ().tset.end ())
        {
          if (owner ().tset.size () == 0)
            return true;
  ...
  insert (typename from_impl::iterator fit, typename to_impl::iterator tit,
    ...
    if (!from.adjacent (fit.fit, x.first))
      {
        ...
        if (fit != fset.end ())
          { /* small chance x is already inserted */
            tset_iterator tnit = tset.find (&x.second);
            if (tnit != tset.end ()
                && element_by_from (*(fit.fit)) == element_by_to (*tnit))
              {
...
protected:
  prebimap (const from_compare &from_comp, const to_compare &to_comp,
            const allocator_type_ &al)
      : allocator (al), fset (p_compare<from_type, from_compare> (from_comp),
                              fset_allocator_type (allocator)),
        tset (p_compare<to_type, to_compare> (to_comp),
              tset_allocator_type (allocator))
  {
  }
    ...
    try
      {
         ...
         * right *after* the insertion point, in disagreement with the
         * standard.
         */

        typename from::iterator fhint = from.end ();
        typename to::iterator thint = to.end ();
        for (typename from::const_iterator it = r.from.begin ();
             it != r.from.end (); ++it)
          {
        ...
        for (typename from_impl::const_iterator it = r.from.begin ();
             it != r.from.end (); ++it)
          {
            std::pair<typename from_impl::iterator, typename to_impl::iterator>
...
template <typename from_type, typename to_type, typename from_compare,
          typename to_compare, typename allocator_type>
struct prebimap_identity
{
  typedef prebimap<from_type, to_type, from_compare, to_compare,
                   allocator_type>
      type;
};
#endif
...
#ifdef __GNUC__
    public prebimap_identity<from_type_, to_type_, from_compare, to_compare,
                             allocator_type_>::type
#else
    public prebimap<from_type_, to_type_, from_compare, to_compare,
                    allocator_type_>
#endif

{
  typedef prebimap<from_type_, to_type_, from_compare, to_compare,
                   allocator_type_>
      super;
  typedef typename super::template from_binding<super> from_binding_t;
  typedef typename super::template const_from_binding<super>
...
public:
  ...
  get_allocator () const
  {
    return this->from.get_allocator ();
  }
  from_binding_t
  ...
  insert (const value_type &x)
  {
    return this->from.insert (x);
  }
  iterator
  insert (iterator it, const value_type &x)
  {
    return this->from.insert (it, x);
  }
  template <typename it_type>
  void
  insert (it_type first, it_type last)
  {
    this->from.insert (first, last);
  }
  ...
  erase (iterator first, iterator last)
  {
    this->from.erase (first, last);
  }
  size_type
  erase (const key_type &key)
  {
    return this->from.erase (key);
  }
  void
  clear ()
  {
  ...
  void
  swap (bimap_equal_types &x)
  {
    this->from.swap (x.from);
  }
  friend void
  swap (bimap_equal_types &x, bimap_equal_types &y)
  {
    x.swap (y);
  }
  key_compare
  key_comp () const
  {
    return this->from.key_comp ();
  }
  value_compare
  value_comp () const
  {
    return this->from.value_comp ();
  }
  iterator
  find (const key_type &key)
  {
    return this->from.find (key);
  }
  const_iterator
  find (const key_type &key) const
  {
  ...
  count (const key_type &key) const
  {
    return this->from.count (key);
  }
  iterator
  lower_bound (const key_type &key)
  {
    return this->from.lower_bound (key);
  }
  const_iterator
  lower_bound (const key_type &key) const
  {
  ...
  upper_bound (const key_type &key)
  {
    return this->from.upper_bound (key);
  }
  const_iterator
  upper_bound (const key_type &key) const
  {
  ...
  equal_range (const key_type &key)
  {
    return this->from.equal_range (key);
  }
  std::pair<const_iterator, const_iterator>
  equal_range (const key_type &key) const
  {
  ...
  std::pair<to_iterator, bool>
  insert (const to_value_type &x)
  {
    return this->to.insert (x);
  }
  to_iterator
  insert (to_iterator it, const to_value_type &x)
  {
    return this->to.insert (it, x);
  }
  void
  insert (const to_value_type *first, const to_value_type *last)
  {
  ...
  {
    this->to.erase (first, last);
  }

protected:
  bimap_equal_types (const from_compare &from_comp, const to_compare &to_comp,
                     const allocator_type_ &al)
...
public:
  ...
  void
  swap (bimap_different_types &x)
  {
    this->from.swap (x.from);
  }
  friend void
  swap (bimap_different_types &x, bimap_different_types &y)
  {
    x.swap (y);
  }
  key_compare
  ...
  to_binding_t
  operator[] (const to_key_type &key)
  {
    return this->to[key];
  }
  const_to_binding_t
  operator[] (const to_key_type &key) const
  {
    return this->to[key];
  }
  std::pair<to_iterator, bool>
  ...
  size_type
  erase (const to_key_type &key)
  {
    return this->to.erase (key);
  }
  to_iterator
  find (const to_key_type &key)
  {
    return this->to.find (key);
  }
  const_to_iterator
  find (const to_key_type &key) const
  {
    return this->to.find (key);
  }
  size_type
  count (const to_key_type &key) const
  {
    return this->to.count (key);
  }
  to_iterator
  lower_bound (const to_key_type &key)
  {
    return this->to.lower_bound (key);
  }
  const_to_iterator
  lower_bound (const to_key_type &key) const
  {
    return this->to.lower_bound (key);
  }
  to_iterator
  upper_bound (const to_key_type &key)
  {
    return this->to.upper_bound (key);
  }
  const_to_iterator
  upper_bound (const to_key_type &key) const
  {
    return this->to.upper_bound (key);
  }
  std::pair<to_iterator, to_iterator>
  equal_range (const to_key_type &key)
  {
    return this->to.equal_range (key);
  }
  std::pair<const_to_iterator, const_to_iterator>
...
protected:
  bimap_different_types (const from_compare &from_comp,
                         const to_compare &to_comp, const allocator_type_ &al)
      : super (from_comp, to_comp, al)
  {
  }
...
template <typename from_type_, typename to_type_,
          ...
          = std::allocator<direct_pair<const from_type_, const to_type_> > >
class bimap : public bimap_detail::select<
                  bimap_detail::equal_types<from_type_, to_type_>::value,
                  bimap_equal_types<from_type_, to_type_, from_compare,
                                    to_compare, allocator_type>,
                  bimap_different_types<from_type_, to_type_, from_compare,
                                        to_compare, allocator_type> >::result
{
protected:
  typedef typename bimap_detail::select<
      bimap_detail::equal_types<from_type_, to_type_>::value,
      bimap_equal_types<from_type_, to_type_, from_compare, to_compare,
                        allocator_type>,
      bimap_different_types<from_type_, to_type_, from_compare, to_compare,
                            allocator_type> >::result super;

public:
  explicit bimap (const from_compare &from_comp = from_compare (),
  ...
  typedef bimap<
      to_type_, from_type_, to_compare, from_compare,
...
#else
  typedef bimap<to_type_, from_type_, to_compare, from_compare,
    ...
    try
      {
        ...
        for (typename inv_bimap::to::const_iterator it = r.to.begin ();
             it != r.to.end (); ++it)
          {
            insert (fhint, thint, *it);
          }
        ...
        for (typename inv_bimap::to::const_iterator it = r.to.begin ();
             it != r.to.end (); ++it)
          {
            std::pair<typename super::from::iterator,
                      typename super::to::iterator>
  ...
  template <typename it_type>
  bimap (it_type first, it_type last,
