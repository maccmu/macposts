    ...
    std::unordered_map<TInt, TInt> &output_map)
{
  ...
  // Initializaiton above. Dijkstra with binary min-heap
  // (std::prioitiry_queue) below:

  // NOTE: Since C++ std::priority_queue does not have decrease_key()
  // function, we insert [pointer to new MNM_cost object] to the min-heap
  // every time when the dist_to_dest[] changes for some node. So there could
  // be duplicated elements in the min-heap for the same nodes with different
  // distance values. But the duplication doesn't affect the correctness of
  // algorithm. (visited label for eliminating the duplication is also
  // tested, but slower than not using it, kind of weird.)
  while (m_Q.size () != 0)
    {
      ...
      for (int e = 0; e < _node_it.GetInDeg (); ++e)
        {
          ...
          if (_alt < _dist.find (_in_node_ID)->second)
            {
              // m_Q.push_back(m_Q_support.find(_in_node_ID) ->
              // second);
...
int
MNM_Shortest_Path::all_to_one_LIFO (TInt dest_node_ID, PNEGraph graph,
                                    std::unordered_map<TInt, TFlt> &cost_map,
                                    std::unordered_map<TInt, TInt> &output_map)
{
  // if (output_map.size() != 0){
...
/*------------------------------------------------------------
                  TDSP  one destination tree
-------------------------------------------------------------*/
MNM_TDSP_Tree::MNM_TDSP_Tree (TInt dest_node_ID, PNEGraph graph,
                              TInt max_interval)
{
...
MNM_TDSP_Tree::~MNM_TDSP_Tree ()
{
  ...
  for (auto _node_it = m_graph->BegNI (); _node_it < m_graph->EndNI ();
    ...
    {
      _node_ID = _node_it.GetId ();
      if (m_dist[_node_ID] != NULL)
        delete m_dist[_node_ID];
      if (m_tree[_node_ID] != NULL)
...
int
MNM_TDSP_Tree::update_tree (std::unordered_map<TInt, TFlt *> &cost_map)
{
