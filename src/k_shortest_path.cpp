...
KSP_Heap::KSP_Heap () { m_top_cost = NULL; }
...
MNM_KSP_Tree::MNM_KSP_Tree (TInt dest_node_ID, PNEGraph graph)
{
  ...
  m_node_ID_ksp_heap_map
      = std::unordered_map<TInt, heap<KSP_Heap *>::node *> ();
}
...
int
MNM_KSP_Tree::initialize ()
{
  return 0;
}

int
MNM_KSP_Tree::update_tree (std::unordered_map<TInt, TFlt> &cost_map)
{
  MNM_Shortest_Path::all_to_one_Dijkstra (
      m_dest_node_ID, m_graph, m_dist_to_dest_map, cost_map, m_sp_tree_map);
  update_DG (cost_map);
...
int
MNM_KSP_Tree::get_ksp (TInt src_node_ID, TInt k, MNM_Pathset *path_set)
{
  // std::deque<TInt> _Q = std::deque<TInt>();
  std::priority_queue<MNM_Path *, std::vector<MNM_Path *>, LessByPathP> _Q
      = std::priority_queue<MNM_Path *, std::vector<MNM_Path *>,
                            LessByPathP> ();
...
int
MNM_KSP_Tree::update_DG (std::unordered_map<TInt, TFlt> &cost_map)
{
  // construct D(G)
  ...
  for (auto _map_it = m_sp_tree_map.begin (); _map_it != m_sp_tree_map.end ();
       ++_map_it)
    {
      ...
      for (int e = 0; e < m_graph->GetNI (_temp_node_ID).GetOutDeg (); ++e)
        {
          ...
          if (_temp_link_ID != _map_it->second)
            {
              _temp_cost = new MNM_Cost (_temp_link_ID,
                                         delta (_temp_link_ID, cost_map));
              _temp_node
                  = _temp_ksp_heap->m_binary_heap_cost.insert (_temp_cost);
              m_link_ID_heap_map.insert ({ _temp_link_ID, _temp_node });
            }
        }
      _temp_ksp_heap->m_top_cost
          = _temp_ksp_heap->m_binary_heap_cost.minimal ();
...
TFlt
MNM_KSP_Tree::delta (TInt link_ID, std::unordered_map<TInt, TFlt> &cost_map)
{
  auto _link_it = m_graph->GetEI (link_ID);
  return cost_map[link_ID] + m_dist_to_dest_map[_link_it.GetDstNId ()]
         - m_dist_to_dest_map[_link_it.GetSrcNId ()];
}

MNM_Path *
MNM_KSP_Tree::build_path_with_one_more_sidetrack (MNM_Path *path)
{
  MNM_Cost *_temp_cost;
  TInt _last_link_ID = path->m_link_vec.back ();
  TInt _head_node_ID = m_graph->GetEI (_last_link_ID).GetDstNId ();
  if (m_node_ID_ksp_heap_map.find (_head_node_ID)
      != m_node_ID_ksp_heap_map.end ())
    {
...
std::vector<MNM_Path *>
MNM_KSP_Tree::build_path_with_except_last_sidetrack (MNM_Path *path)
{
