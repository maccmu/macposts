...
MNM_Veh_Delivery::~MNM_Veh_Delivery() {
  m_multi_od_seq = nullptr;
}
...
std::pair<MNM_Origin*, MNM_Destination*> MNM_Veh_Delivery::get_current_od()
{
...
int MNM_Veh_Delivery::move_to_next_od()
{
...
MNM_Veh_Delivery*
MNM_Veh_Factory_Delivery::make_veh_delivery(TInt timestamp, Vehicle_type veh_type)
{
...
MNM_Dlink_Pq_Delay::MNM_Dlink_Pq_Delay(
    ...
    TFlt unit_time,
    TFlt flow_scalar)
    : MNM_Dlink_Pq::MNM_Dlink_Pq(ID, lane_hold_cap, lane_flow_cap, number_of_lane,
...
MNM_Dlink *
MNM_Link_Factory_Delivery::make_link (TInt ID, DLink_type link_type, TFlt lane_hold_cap,
                             ...
                             TFlt flow_scalar)
{
  ...
  switch (link_type)
    ...
    case MNM_TYPE_PQ:
      _link
        = new MNM_Dlink_Pq_Delay (ID, lane_hold_cap, lane_flow_cap, number_of_lane,
                            length, ffs, unit_time, flow_scalar);
      break;
    case MNM_TYPE_LQ:
      _link
...
MNM_Origin_Delivery::MNM_Origin_Delivery(TInt ID, TInt max_interval, TFlt flow_scalar, TInt frequency)
...
int 
MNM_Origin_Delivery::add_multi_OD_seq_demand(std::vector<std::pair<MNM_Origin*, MNM_Destination*>> *multi_od_seq, TFlt* demand)
{
...
int MNM_Origin_Delivery::release_one_interval(TInt current_interval, MNM_Veh_Factory* veh_factory,
                                              TInt assign_interval, TFlt adaptive_ratio)
{
  ...
  auto _veh_factory_deliver = dynamic_cast<MNM_Veh_Factory_Delivery*>(veh_factory);
  ...
  for (auto _demand_it = m_demand.begin(); _demand_it != m_demand.end(); _demand_it++) {
    ...
    for (int i = 0; i < _veh_to_release; ++i) {
      ...
      _veh = _veh_factory_deliver -> make_veh(current_interval, MNM_Ults::rand_flt() <= adaptive_ratio ? MNM_TYPE_ADAPTIVE : MNM_TYPE_STATIC);
  ...
  for (auto _demand_it : m_demand_multi_OD_seq) {
    ...
    for (int i = 0; i < _veh_to_release; ++i) {
      ...
      _label = generate_label(0);
      _veh_deliver = _veh_factory_deliver -> make_veh_delivery(current_interval, MNM_Ults::rand_flt() <= adaptive_ratio ? MNM_TYPE_ADAPTIVE : MNM_TYPE_STATIC);
      ...
      // _veh -> m_assign_interval = assign_interval;
      // in case the multiclass modeling has 1-min release interval as the "assign" interval
      _veh_deliver -> m_assign_interval = int(current_interval / m_frequency);
...
MNM_Destination_Delivery::receive(TInt current_interval)
{
  ...
  for (size_t i=0; i < _num_to_receive; ++i){
    ...
    if (auto _veh_deliver = dynamic_cast<MNM_Veh_Delivery*>(_veh)) {
        if (_veh_deliver -> m_multi_od_seq != nullptr && _veh_deliver -> m_multi_od_seq -> back().second != _veh_deliver -> get_current_od().second) {
            Assert(_veh_deliver -> get_current_od().second == this);
            _veh_deliver -> move_to_next_od();
            Assert(_veh_deliver -> get_current_od().second != this);
            // delay some time
            _veh_deliver -> m_waiting_time = dynamic_cast<MNM_Origin_Delivery*>(_veh_deliver -> m_origin) -> m_pickup_waiting_time;
            _veh_deliver -> m_origin -> m_origin_node -> m_in_veh_queue.push_back(_veh_deliver);
...
int 
MNM_Destination_Delivery::receive(TInt current_interval, MNM_Routing *routing, MNM_Veh_Factory *veh_factory, bool del)
{
  MNM_Veh *_veh;
  size_t _num_to_receive = m_dest_node -> m_out_veh_queue.size();
  // printf("Dest node %d out vehicle: %d\n", m_dest_node -> m_node_ID, _num_to_receive);
  for (size_t i=0; i < _num_to_receive; ++i){
    ...
    if (auto _veh_deliver = dynamic_cast<MNM_Veh_Delivery*>(_veh)) {
        if (_veh_deliver -> m_multi_od_seq != nullptr && _veh_deliver -> m_multi_od_seq -> back().second != _veh_deliver -> get_current_od().second) {
            ...
            _veh_deliver -> m_waiting_time = dynamic_cast<MNM_Origin_Delivery*>(_veh_deliver -> m_origin) -> m_pickup_waiting_time;
            _veh_deliver -> m_origin -> m_origin_node -> m_in_veh_queue.push_back(_veh_deliver);
            routing -> remove_finished(_veh_deliver, del);  // remove STATIC users from m_tracker
            _veh_deliver -> m_path = nullptr;
        }
        else {
            _veh_deliver -> finish(current_interval);
      ...
      _veh -> finish(current_interval);
      routing -> remove_finished(_veh, del);  // remove STATIC users from m_tracker
...
MNM_Destination *MNM_OD_Factory_Delivery::make_destination(TInt ID)
{
...
MNM_Origin *MNM_OD_Factory_Delivery::make_origin(TInt ID, TInt max_interval, TFlt flow_scalar, TInt frequency)
{
  MNM_Origin_Delivery *_origin = new MNM_Origin_Delivery(ID, max_interval, flow_scalar, frequency);
...
MNM_Routing_Delivery_Fixed::MNM_Routing_Delivery_Fixed (PNEGraph &graph,
    ...
    : MNM_Routing_Fixed::MNM_Routing_Fixed (graph, od_factory, node_factory,
                                            link_factory, routing_frq, buffer_len)
{
  ;
}
...
int MNM_Routing_Delivery_Fixed::remove_finished(MNM_Veh* veh, bool del)
{
...
MNM_Routing_Delivery_Hybrid::MNM_Routing_Delivery_Hybrid (
  ...
  MNM_Link_Factory *link_factory, TInt route_frq_fixed, TInt buffer_len)
    : MNM_Routing_Hybrid::MNM_Routing_Hybrid (file_folder, graph, statistics,
                                              ...
                                              link_factory, route_frq_fixed, buffer_len)
{
...
{
  auto _od_factory_delivery = dynamic_cast<MNM_OD_Factory_Delivery*>(od_factory);
...
int  
MNM_IO_Delivery::build_od_factory_delivery(const std::string& file_folder, MNM_ConfReader *conf_reader,
  ...
  if (_od_file.is_open())
  {
    ...
    for (int i=0; i < _num_of_O; ++i){
      ...
      if (_words.size() == 2 || _words.size() == 3) {
        ...
        _origin_ID = TInt(std::stoi(_words[0]));
        _node_ID = TInt(std::stoi(_words[1]));
        _od_factory_delivery -> make_origin(_origin_ID, _max_interval, _flow_scalar, _frequency);
        if (_words.size() == 3) {
          _pickup_waiting_time = TFlt(std::stod(_words[2]));  // second
          dynamic_cast<MNM_Origin_Delivery*>(_od_factory_delivery -> get_origin(_origin_ID)) -> m_pickup_waiting_time = MNM_Ults::round(_pickup_waiting_time / _unit_time);  // intervals
        }
        else {
          dynamic_cast<MNM_Origin_Delivery*>(_od_factory_delivery -> get_origin(_origin_ID)) -> m_pickup_waiting_time = 0;
        }
      }
    }
    std::getline(_od_file,_line); // skip another line
...
int 
MNM_IO_Delivery::build_demand_multi_OD_seq(const std::string& file_folder, MNM_ConfReader *conf_reader,
                                MNM_OD_Factory *od_factory, 
  ...
  if (_od_file.is_open() && _demand_file.is_open())
  {
    ...
    for (int i=0; i < _num_multiOD; ++i){
      ...
      for (size_t i=0; i<_words.size(); i+=2) {
        ...
        _origin = dynamic_cast<MNM_Origin_Delivery*>(_od_factory_delivery -> get_origin(_O_ID));
        _dest = dynamic_cast<MNM_Destination_Delivery*>(_od_factory_delivery -> get_destination(_D_ID));
...
MNM_Dta_Delivery::set_routing()
{
    if (m_config -> get_string("routing_type") == "Hybrid"){
        ...
        m_routing = new MNM_Routing_Delivery_Hybrid(m_file_folder, m_graph, m_statistics, m_od_factory, m_node_factory, 
