...
int
MNM_Cumulative_Emission::update ()
{
  ...
  for (MNM_Dlink *link : m_link_vector)
    {
      ...
      m_fuel += calculate_fuel_rate (_v_converted)
                * (_v * m_unit_time / TFlt (1600)) * link->get_link_flow ();
      ...
      // printf("link ID is %d, flow is :%lf, _v is %lf, CO2 is %lf, HC
      // is %lf\n",
      //       (link -> m_link_ID)(), link -> get_link_flow()(), _v(),
      //       m_CO2(), m_HC());
    }
